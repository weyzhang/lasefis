 ...
 * ============================================================================
 */
#define TF_CSEIFE_TIDES_C_VERSION "TF_CSEIFE_TIDES_C   V1.0   "
...
void seife_tid(double* x, int n, double dt, int nstep) {
  ...
  for (j = 1; j <= i__2; ++j) {
    /* L102: */
    cor[j - 1] = step * sin(omeg[j - 1] * dth) / sin(step * omeg[j - 1] * dth);
  }
  ...
  for (j = 1; i__3 < 0 ? j >= i__4 : j <= i__4; j += i__3) {
    ...
    for (k = 1; k <= i__1; ++k) {
      k2 = k << 1;
      k3 = k2 + 1;
      xgez1 += cor[k - 1] * (f[k2 - 1] * c__[k2 - 1] + f[k3 - 1] * c__[k3 - 1]);
      xgez2 += cor[k - 1] * (f[k2 - 1] * d__[k2 - 1] + f[k3 - 1] * d__[k3 - 1]);
      /* L105: */
      xgez3 +=
          cor[k - 1] * (f[k2 - 1] * e[k2 - 1] + f[k3 - 1] * e[k3 - 1]) / two;
    }
    if (j > n - (nstep << 1) + 1) {
      nstep = n + 1 - j;
    }
