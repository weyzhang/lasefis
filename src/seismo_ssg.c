...
void seismo(int lsamp, int ntr, int** recpos, float** sectionvx,
  ...
  ins =
      lsamp; /* changed from "ins=lsamp/NDT;" (neccessary after correction of
                the buggy ns in ifos.c) */
  dh24x = 1.0 / DX;
  dh24y = 1.0 / DY;
  dh24z = 1.0 / DZ;
  ...
  for (itr = 1; itr <= ntr; itr++) {
    ...
    switch (SEISMO) {
      case 1:
        ...
        break;
      case 2:
        sectionp[itr][ins] = -sxx[nyrec][nxrec][nzrec] -
      ...
      case 3:
        ...
        amp = u[j][i][k] *
              ((vyz - vzy) * fabs(vyz - vzy) + (vzx - vxz) * fabs(vzx - vxz) +
               (vxy - vyx) * fabs(vxy - vyx));
