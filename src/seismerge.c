...
/*------------------------------------------------------------------------
 *  merging SEG-Y files with non-native endian quick and dirty (The SEG-Y header
 *of
...
int main(int argc, char** argv) {
  ...
  unsigned short nsamp, msamp, ntraces = 0,
                               mtraces; /* mixed short and unsigned short */
  n = argc;
  if (n == 1) {
    fprintf(stdout, " usage of seismerge (merging SEG-Y files):\n");
    fprintf(stdout,
            " seismerge segyfile-name_without_.cpu-no. [maximum_cpu-no. "
            "[output-filename]]\n\n");
    fprintf(stdout,
            " Path length (input and output file) must not exceed 255 "
            "characters.\n");
  ...
  if (strlen(basename) > 255) {
    fprintf(stdout,
            " seismerge cannot handle path with more than 255 characters.\n");
    return 0;
  }
  if (n > 2) {
    sscanf(argv[2], "%d", &maxcpu);
    if (maxcpu == 0)
      fprintf(stdout, "Error: seismerge could not read max. number of cpus.\n");
  }
  if (maxcpu == 0)
    fprintf(stdout, " Warning: Setting maximum CPU-no. to default value: %d.\n",
            maxcpu = 1000);
  if (n > 3)
    outname = argv[3];
  else
    outname = basename;
  if ((outfile = fopen(outname, "w+")) != NULL) /* fprintf(stderr, "\t File %s
                                                   opend for writing. ...
                                                   \n",outname) */
    ;
  else {
  ...
  if ((infiles = fopen(inname, "r+")) != NULL) {
    ...
    if (nsamp != msamp)
      fprintf(stdout,
              "Warning: Number of samples in SEG-Y header and 1st trace header "
              "differ?!\n");
    ...
    fclose(infiles);
    /* fprintf(stderr, "\t File %s with %d traces (%d bytes) has been copied to
  ...
  for (n++; n < maxcpu; n++) {
    ...
    if ((infiles = fopen(inname, "r+")) != NULL) {
      ...
      if (nsamp != msamp)
        fprintf(stdout,
                "Warning: Number of samples in SEG-Y header and 1st trace "
                "header differ?!\n");
      if (nsamp != 0) { /* -> ns from SEG-Y header */
        fseek(infiles, -3600, SEEK_END);
        msamp = nsamp;
      ...
      fclose(infiles);
      /* fprintf(stderr, "\t File %s has been appended to file %s (%d bytes
       * appended, now %d traces). ... \n",inname,outname,b,ntraces); */
      m++;
    }
  }
  ...
  for (l = 1; l <= ntraces; l++) {
    n = ((l >> 24) & 0xff) | ((l & 0xff) << 24) | ((l >> 8) & 0xff00) |
        ((l & 0xff00) << 8);
  ...
  free(inname);
  fprintf(stderr,
          " %d traces in %d files (with basename %s) \n merged into one file "
          "(%s). \n",
