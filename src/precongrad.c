...
void precon_grad(int nx, int ny, int nz, float*** grad1, float*** grad2,
  ...
  int i, j, k, l, n, ii = 0, kk = 0, jj = 0, sx = 0, sy = 0, sz = 0, rx, ry,
                     rz;  //,h;
  int sh,
      rh; /*distance of source and receiver arrays to model edge (grid points)*/
  float r = 0.0, G[3];
  G[0] = 0.0;
  G[1] = 0.0;
  G[2] = 0.0;
      ...
      for (i = 1; i <= nx; i++) {
        for (k = 1; k <= nz; k++) {
          ...
          for (n = 1; n <= ntr_glob; n++) {
            ...
            rz = recpos[3][n];
            r = 0.0;
            r = sqrt((jj - ry) * (jj - ry) + (ii - rx) * (ii - rx) +
                     (kk - rz) * (kk - rz));
            G[0] = 0.0;
            G[1] = 0.0;
  ...
  if (DAMPTYPE == 2) {
    /* circular damping around source and receiver positions and damping in
     * model boundaries */
    for (j = 1; j <= ny; j++) {
      for (i = 1; i <= nx; i++) {
        for (k = 1; k <= nz; k++) {
          ...
          for (l = 1; l <= nsrc; l++) {
            ...
            r = 0.0;
            r = sqrt((jj - sy) * (jj - sy) + (ii - sx) * (ii - sx) +
                     (kk - sz) * (kk - sz));
  ...
  if (DAMPTYPE == 3) {
    /*damping source and receiver plane and model boundaries (used for
