...
void absorb(float*** absorb_coeff) {
  ...
  if (MYID == 0) {
    ...
    if ((((NX) < FW) || ((NY) < FW)) || ((NZ) < FW)) {
      fprintf(FP,
              " \n Width of absorbing boundary (ABS=%i gridpoints) is larger "
              "than at least one subdomain dimension: \n",
              FW);
      fprintf(
          FP,
          " \t NX/NPROCX = %i, NY/NPROCY = %i, NZ/NPROCZ = %i gridpoints.\n",
          NX, NY, NZ);
      err(" Choose smaller width of absorbing frame (ABS) or increase "
  ...
  for (i = 1; i <= ifw; i++) coeff[i] = exp(-(a * a * (ifw - i) * (ifw - i)));

  if (MYID == 0) {
    fprintf(FP, " Table of coefficients \n # \t coeff \n");
    /*fprintf(FP," ifw=%d \t a=%f amp=%f \n", ifw,a,amp); */
    for (i = 1; i <= ifw; i++) fprintf(FP, " %d \t %5.3f \n", i, coeff[i]);
  }
  ...
  if ((!BOUNDARY) && (POS[1] == 0)) {
    ...
    for (i = 1; i <= ifw; i++) {
      ...
      for (k = zb; k <= ze; k++)
        for (j = yb; j <= ye; j++) absorb_coeff[j][i][k] = coeff[i];
    }
  }
  ...
  if ((!BOUNDARY) && (POS[1] == NPROCX - 1)) {
    ...
    for (i = 1; i <= ifw; i++) {
      ...
      if (POS[2] == NPROCY - 1) ye = NY - i + 1;
      for (k = zb; k <= ze; k++)
        for (j = yb; j <= ye; j++) absorb_coeff[j][ii][k] = coeff[i];
    }
  }
  ...
  if ((!BOUNDARY) && (POS[3] == 0)) {
    ...
    for (k = 1; k <= ifw; k++) {
      ...
      for (i = xb; i <= xe; i++)
        for (j = yb; j <= ye; j++) absorb_coeff[j][i][k] = coeff[k];
    }
  }
  ...
  if ((!BOUNDARY) && (POS[3] == NPROCZ - 1)) {
    ...
    for (k = 1; k <= ifw; k++) {
      ...
      if (POS[2] == NPROCY - 1) ye = NY - k + 1;
      for (i = xb; i <= xe; i++)
        for (j = yb; j <= ye; j++) absorb_coeff[j][i][kk] = coeff[k];
    }
  }
  ...
  if ((POS[2] == 0) && (!(FREE_SURF))) {
    ...
    for (j = 1; j <= ifw; j++) {
      ...
      for (i = xb; i <= xe; i++)
        for (k = zb; k <= ze; k++) absorb_coeff[j][i][k] = coeff[j];
    }
  }
  ...
  if (POS[2] == NPROCY - 1) {
    ...
    for (j = 1; j <= ifw; j++) {
      ...
      if ((!BOUNDARY) && (POS[3] == NPROCZ - 1)) ze = NZ - j + 1;
      for (i = xb; i <= xe; i++)
        for (k = zb; k <= ze; k++) absorb_coeff[jj][i][k] = coeff[j];
    }
  }
