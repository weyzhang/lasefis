...
double exchange_s(float*** sxx, float*** syy, float*** szz, float*** sxy,
  ...
  MPI_Bsend(&buffertop_to_bot[1][1][1], NX * NZ * nf2, MPI_FLOAT, INDEX[3],
            TAG5, MPI_COMM_WORLD);
  MPI_Barrier(MPI_COMM_WORLD);
  MPI_Recv(&buffertop_to_bot[1][1][1], NX * NZ * nf2, MPI_FLOAT, INDEX[4], TAG5,
           MPI_COMM_WORLD, &status);
  MPI_Bsend(&bufferbot_to_top[1][1][1], NX * NZ * nf1, MPI_FLOAT, INDEX[4],
            TAG6, MPI_COMM_WORLD);
  MPI_Barrier(MPI_COMM_WORLD);
  MPI_Recv(&bufferbot_to_top[1][1][1], NX * NZ * nf1, MPI_FLOAT, INDEX[3], TAG6,
           MPI_COMM_WORLD, &status);
  ...
  MPI_Bsend(&bufferlef_to_rig[1][1][1], NY * NZ * nf2, MPI_FLOAT, INDEX[1],
            TAG1, MPI_COMM_WORLD);
  MPI_Barrier(MPI_COMM_WORLD);
  MPI_Recv(&bufferlef_to_rig[1][1][1], NY * NZ * nf2, MPI_FLOAT, INDEX[2], TAG1,
           MPI_COMM_WORLD, &status);
  MPI_Bsend(&bufferrig_to_lef[1][1][1], NY * NZ * nf1, MPI_FLOAT, INDEX[2],
            TAG2, MPI_COMM_WORLD);
  MPI_Barrier(MPI_COMM_WORLD);
  MPI_Recv(&bufferrig_to_lef[1][1][1], NY * NZ * nf1, MPI_FLOAT, INDEX[1], TAG2,
           MPI_COMM_WORLD, &status);
  ...
  MPI_Bsend(&bufferfro_to_bac[1][1][1], NX * NY * nf2, MPI_FLOAT, INDEX[5],
            TAG3, MPI_COMM_WORLD);
  MPI_Barrier(MPI_COMM_WORLD);
  MPI_Recv(&bufferfro_to_bac[1][1][1], NX * NY * nf2, MPI_FLOAT, INDEX[6], TAG3,
           MPI_COMM_WORLD, &status);
  MPI_Bsend(&bufferbac_to_fro[1][1][1], NX * NY * nf1, MPI_FLOAT, INDEX[6],
            TAG4, MPI_COMM_WORLD);
  MPI_Barrier(MPI_COMM_WORLD);
  MPI_Recv(&bufferbac_to_fro[1][1][1], NX * NY * nf1, MPI_FLOAT, INDEX[5], TAG4,
           MPI_COMM_WORLD, &status);
  ...
  /*if (LOG)
      if (MYID==0){
              time2=MPI_Wtime();
              time=time2-time1;
              fprintf(FP," Real time for stress tensor exchange: \t\t %4.2f
     s.\n",time);
      }*/
  return time;
}
