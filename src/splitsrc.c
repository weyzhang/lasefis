...
float** splitsrc(float** srcpos, int* nsrc_loc, int nsrc, int* snum_loc) {
  ...
  for (j = 1; j <= nsrc; j++) {
    ...
    if ((POS[1] == a) && (POS[2] == b) && (POS[3] == c)) {
      i++;
      srcpos_dummy[1][i] =
          (float)(((iround(srcpos[1][j] / DX) - 1) % IENDX) + 1);
      srcpos_dummy[2][i] =
          (float)(((iround(srcpos[2][j] / DY) - 1) % IENDY) + 1);
      srcpos_dummy[3][i] =
          (float)(((iround(srcpos[3][j] / DZ) - 1) % IENDZ) + 1);
      srcpos_dummy[4][i] = srcpos[4][j];
      srcpos_dummy[5][i] = srcpos[5][j];
      srcpos_dummy[6][i] = srcpos[6][j];
      srcpos_dummy[7][i] = srcpos[7][j];
      ; /* stype_loc might be scaled for minimum size by realloc later */
      snum_loc[j] = 1;
    } else
      snum_loc[j] = 0;
  }
  ...
  if (!RUN_MULTIPLE_SHOTS) {
    if (VERBOSE) {
      ...
      fprintf(FP,
              " Splitting of source positions from global to local grids "
              "finished.\n");
      fprintf(FP, " MYID= %d \t \t no. of sources= %d\n", MYID, i);
      if (MYID == 0) {
        printf("\n **Message from splitsrc:\n");
        printf(
            " Table of local source positions (in gridpoints), time-shift, "
            "centre frequency, amplitude and source type:\n");
    ...
    for (j = 1; j <= i; j++)
      printf(" %3d\t%4.0f\t%4.0f\t%4.0f\t%6.2f\t%6.2f\t%6.2f\t%4.2f\n", MYID,
             srcpos_local[1][j], srcpos_local[2][j], srcpos_local[3][j],
