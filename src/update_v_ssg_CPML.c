...
double update_v_CPML(int nx1, int nx2, int ny1, int ny2, int nz1, int nz2,
  ...
  float sxx_x = 0.0, sxy_y = 0.0, sxz_z = 0.0, syy_y = 0.0, sxy_x = 0.0,
        syz_z = 0.0;
    ...
    for (j = 1; j <= NY; j++) {
      for (i = 1; i <= FW; i++) {
        for (k = 1; k <= NZ; k++) {
          ...
          psi_sxx_x[j][i][k] =
              b_x_half[i] * psi_sxx_x[j][i][k] + a_x_half[i] * sxx_x;
          ...
          if ((POS[2] == 0 && FREE_SURF == 0) && (j <= FW)) {
            ...
            psi_syy_y[j][i][k] =
                b_y_half[j] * psi_syy_y[j][i][k] + a_y_half[j] * syy_y;
          ...
          if ((POS[2] == NPROCY - 1) && (j >= ny2 + 1)) {
            h1 = (j - ny2 + FW);
            psi_sxy_y[h1][i][k] =
                b_y[h1] * psi_sxy_y[h1][i][k] + a_y[h1] * sxy_y;
            sxy_y = sxy_y / K_y[h1] + psi_sxy_y[h1][i][k];
            psi_syy_y[h1][i][k] =
                b_y_half[h1] * psi_syy_y[h1][i][k] + a_y_half[h1] * syy_y;
            syy_y = syy_y / K_y_half[h1] + psi_syy_y[h1][i][k];
            psi_syz_y[h1][i][k] =
                b_y[h1] * psi_syz_y[h1][i][k] + a_y[h1] * syz_y;
          ...
          if ((POS[3] == 0) && (k <= FW)) {
            ...
            psi_szz_z[j][i][k] =
                b_z_half[k] * psi_szz_z[j][i][k] + a_z_half[k] * szz_z;
          ...
          if ((POS[3] == NPROCZ - 1) && (k >= nz2 + 1)) {
            h1 = (k - nz2 + FW);
            psi_sxz_z[j][i][h1] =
                b_z[h1] * psi_sxz_z[j][i][h1] + a_z[h1] * sxz_z;
            sxz_z = sxz_z / K_z[h1] + psi_sxz_z[j][i][h1];
            psi_syz_z[j][i][h1] =
                b_z[h1] * psi_syz_z[j][i][h1] + a_z[h1] * syz_z;
            syz_z = syz_z / K_z[h1] + psi_syz_z[j][i][h1];
            psi_szz_z[j][i][h1] =
                b_z_half[h1] * psi_szz_z[j][i][h1] + a_z_half[h1] * szz_z;
    ...
    for (j = 1; j <= NY; j++) {
      for (i = nx2 + 1; i <= nx2 + FW; i++) {
        for (k = 1; k <= NZ; k++) {
          ...
          psi_sxx_x[j][h1][k] =
              b_x_half[h1] * psi_sxx_x[j][h1][k] + a_x_half[h1] * sxx_x;
  ...
  /*if (LOG)
      if (MYID==0){
              time2=MPI_Wtime();
              time=time2-time1;
              fprintf(FP," Real time for CPML particle velocity update: \t %4.2f
     s.\n",time);
      }*/
  return time;
}
