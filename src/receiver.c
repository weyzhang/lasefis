  ...
  if (MYID == 0) {
    switch (READREC) { /* read receiver positions from file */
        ...
        *ntr = 0;
        ...
        while (fgets(buffer, STRING_SIZE, fpr)) {
          ...
          /*testbuff=sscanf(buffer,"%s",bufferstring);
                                  fprintf(fp," bufferstring : _%s_with
      ...
      case 2:          /* REC ARRAY */
        ...
        *ntr = ((1 + (NZG - 2 * ifw) / DRZ) * (1 + (NXG - 2 * ifw) / DRX)) *
               REC_ARRAY;
      ...
      case 0: /* straight horizontal or vertical line of receivers */
        ...
        if ((XREC1 > XREC2) || ((YREC1 > YREC2) || (ZREC1 > ZREC2))) {
          fprintf(fp,
                  " Coordinates of first receiver specified in input file :\n");
          fprintf(fp, "    %5.2f (x) , %5.2f (y) , %5.2f (z) :\n", XREC1, YREC1,
                  ZREC1);
          fprintf(fp,
                  " Coordinates of last receiver specified in input file :\n");
          fprintf(fp, "    %5.2f (x) , %5.2f (y) , %5.2f (z) :\n", XREC2, YREC2,
                  ZREC2);
          err("\n\n Receiver coordinates of first receiver should be "
              "equal/smaller than last receiver coordinates!");
        }

        nxrec1 = iround(
            XREC1 / DX); /* (nxrec1,nyrec1,nzrec1) and (nxrec2,nyrec2,nzrec2) */
        nyrec1 = iround(
            YREC1 / DY); /* are the positions of the first and last receiver*/
        ...
        if ((abs(nyrec2 - nyrec1) <= abs(nxrec2 - nxrec1)) ||
          ...
          if (abs(nzrec2 - nzrec1) <= abs(nxrec2 - nxrec1)) {
            ...
            *ntr = iround((nxrec2 - nxrec1) / NGEOPH) + 1;
            ...
            for (nxrec = nxrec1; nxrec <= nxrec2; nxrec += NGEOPH) {
              if ((nyrec2 - nyrec1 == 0) && (nzrec2 - nzrec1 == 0)) {
                ...
                nyrec = nyrec1 + ((nyrec2 - nyrec1) / (nxrec2 - nxrec1) *
                                  (nxrec - nxrec1));
                nzrec = nzrec1 + ((nzrec2 - nzrec1) / (nxrec2 - nxrec1) *
                                  (nxrec - nxrec1));
              }
            ...
            *ntr = iround((nzrec2 - nzrec1) / NGEOPH) + 1;
            ...
            for (nzrec = nzrec1; nzrec <= nzrec2; nzrec += NGEOPH) {
              nyrec = nyrec1 + ((nyrec2 - nyrec1) / (nzrec2 - nzrec1) *
                                (nzrec - nzrec1));
              nxrec = nxrec1 + ((nxrec2 - nxrec1) / (nzrec2 - nzrec1) *
                                (nzrec - nzrec1));
          ...
          *ntr = iround((nyrec2 - nyrec1) / NGEOPH) + 1;
          ...
          for (nyrec = nyrec1; nyrec <= nyrec2; nyrec += NGEOPH) {
            nxrec = nxrec1 +
                    ((nxrec2 - nxrec1) / (nyrec2 - nyrec1) * (nyrec - nyrec1));
            nzrec = nzrec1 +
                    ((nzrec2 - nzrec1) / (nyrec2 - nyrec1) * (nyrec - nyrec1));
  ...
  if (MYID == 0) {
    ...
    for (k = 1; k <= kmax; k++)
      fprintf(fp, " %5.2f   %5.2f   %5.2f\n", recpos[1][k] * DX,
              recpos[2][k] * DY, recpos[3][k] * DZ);
    if (!VERBOSE)
      fprintf(fp, " ...\n Only %d of %d receiver positions displayed here",
              kmax, *ntr);
    fprintf(fp, "\n\n");
  }
