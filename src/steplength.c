...
void steplength(float* L2, float* step, int iteration, int it_group) {
  ...
  a = ((step[0] - step[1]) * (L2[0] - L2[2]) -
       (step[0] - step[2]) * (L2[0] - L2[1])) /
  ...
  if (step[3] < 0.1 * step[1]) {
    step[3] = 0.1 * step[1];
    if (MYID == 0)
      fprintf(FP, " Warning: optimal steplength smaller 0.1*step[1]\n");
  }
  ...
  if (MYID == 0)
    fprintf(FP,
            "\n Steplengthparabel: L2=%.2e*step^2 + %.2e*step + %.2e, Minimum "
            "L2(%.2e)=%.2e\n",
            a, b, c, step[3], L2[3]);
  if (MYID == 0)
    fprintf(FI,
