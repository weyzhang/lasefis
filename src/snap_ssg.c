...
void snap(FILE* fp, int nt, int nsnap, int format, int type, float*** vx,
  ...
  int i, j, k;
  float a = 0.0, amp, dh24x, dh24y, dh24z, vyx, vxy, vxx, vyy, vzx, vyz, vxz,
        vzy, vzz;
  /*extern FILE *FP;*/
  ...
  switch (type) {
    ...
    case 3:
      ...
      for (k = ny1; k <= ny2; k += idy)
        for (i = nx1; i <= nx2; i += idx)
          for (j = nz1; j <= nz2; j += idz) {
            ...
            amp = ((vzy - vyz) * (vzy - vyz) + (vxz - vzx) * (vxz - vzx) +
                   (vyx - vxy) * (vyx - vxy));
            ...
            switch (SNAP_PLANE) {
              case 1: /* energy without sign */
                      /* sqrt(Es) with Es = u*amp*amp (second amp removed due to
                       * missing sqrt in amp*/
                a = sqrt((u[j][i][k]) * amp);
                break;
              case 2: /* energy with sign true for x-y-plane */
                      /*sign(rot(v)x * sqrt(Es) with Es = u*amp*amp (second amp
                       * removed due to missing sqrt in amp*/
                a = fsign((vxy - vyx)) * sqrt((u[j][i][k]) * amp);
                break;
              case 3: /* energy with sign true for x-z-plane */
                      /*sign(rot(v)r * sqrt(Es) with Es = u*amp*amp (second amp
                       * removed due to missing sqrt in amp*/
                a = fsign((vxz - vzx)) * sqrt((u[j][i][k]) * amp);
                break;
              case 4: /* energy with sign true for y-z-plane */
                      /*sign(rot(v)t * sqrt(Es) with Es = u*amp*amp (second amp
                       * removed due to missing sqrt in amp*/
            ...
            /*sign(rot(v)t * sqrt(Es) with Es = u*amp*amp (second amp removed
             * due to missing sqrt in amp*/
            /*a=fsign((vxz-vzx))*sqrt((u[j][i][k])*amp);
      ...
      /* output of the divergence of the velocity field according to Dougherty
         and
                                Stephen (PAGEOPH, 1988) */
      for (k = ny1; k <= ny2; k += idy)
        for (i = nx1; i <= nx2; i += idx)
