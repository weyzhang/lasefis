...
double update_s(int nx1, int nx2, int ny1, int ny2, int nz1, int nz2,
      ...
      for (j = ny1; j <= ny2; j++) {
        for (i = nx1; i <= nx2; i++) {
          for (k = nz1; k <= nz2; k++) {
            ...
            /* computing sums of the old memory variables in this version only
               one mechanism is possi
                            ble */
  ...
  /*if (LOG)
      if (MYID==0){
              time2=MPI_Wtime();
              time=time2-time1;
              fprintf(FP," Real time for stress tensor update: \t\t %4.2f
     s.\n",time);
      }*/
  return time;
}
