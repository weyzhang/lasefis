...
int main(int argc, char **argv) {
  ...
  int ntr = 0, ntr_loc = 0, ntr_glob = 0, nsrc = 0, nsrc_loc = 0, ishot, ishot1,
      nshots;

  double time1 = 0.0, time2 = 0.0, time4 = 0.0;
  double *time_v_update, *time_s_update, *time_s_exchange, *time_v_exchange,
      *time_timestep;
  int *xb, *yb, *zb, l, i, j;
  ...
  float ***uipjp = NULL, ***ujpkp = NULL, ***uipkp = NULL, ***tausipjp = NULL,
        ***tausjpkp = NULL, ***tausipkp = NULL, ***rjp = NULL, ***rkp = NULL,
        ***rip = NULL;

  float **srcpos = NULL, **srcpos_loc = NULL, **srcpos_loc_back = NULL,
        **srcpos1 = NULL, **signals = NULL;
  int **recpos = NULL, **recpos_loc = NULL, *snum_loc = NULL, *rnum_loc = NULL;
  float **sectionvx = NULL, **sectionvy = NULL, **sectionvz = NULL,
        **sectionp = NULL, **sectioncurl = NULL, **sectiondiv = NULL;
  ...
  float *K_x = NULL, *alpha_prime_x = NULL, *a_x = NULL, *b_x = NULL,
        *K_x_half = NULL, *alpha_prime_x_half = NULL, *a_x_half = NULL,
        *b_x_half = NULL, *K_y = NULL, *alpha_prime_y = NULL, *a_y = NULL,
        *b_y = NULL, *K_y_half = NULL, *alpha_prime_y_half = NULL,
        *a_y_half = NULL, *b_y_half = NULL, *K_z = NULL, *alpha_prime_z = NULL,
        *a_z = NULL, *b_z = NULL, *K_z_half = NULL, *alpha_prime_z_half = NULL,
        *a_z_half = NULL, *b_z_half = NULL;
  float ***psi_sxx_x = NULL, ***psi_syy_y = NULL, ***psi_szz_z = NULL,
        ***psi_sxy_y = NULL, ***psi_sxy_x = NULL, ***psi_sxz_x = NULL,
        ***psi_sxz_z = NULL, ***psi_syz_y = NULL, ***psi_syz_z = NULL,
        ***psi_vxx = NULL, ***psi_vyy = NULL, ***psi_vzz = NULL,
        ***psi_vxy = NULL, ***psi_vxz = NULL, ***psi_vyx = NULL,
        ***psi_vyz = NULL, ***psi_vzx = NULL, ***psi_vzy = NULL;

  float **sectionread = NULL, **sectionreadf = NULL, **sectionvxdiff = NULL,
        **sectionvydiff = NULL, **sectionvzdiff = NULL, *misfit;
  ...
  float ****Ffvx = NULL, ****Ffvy = NULL, ****Ffvz = NULL, ****Ffivx = NULL,
        ****Ffivy = NULL, ****Ffivz = NULL;
  float ****Fbvx = NULL, ****Fbvy = NULL, ****Fbvz = NULL, ****Fbivx = NULL,
        ****Fbivy = NULL, ****Fbivz = NULL;
  int iteration = 0, steptest = 0, cdf = 0, groupnum = 0, nf = 0, *itpergroup,
      it_group = 0, itmax = 0, ntast = 1;
  int ntr_hess = 0;
  ...
  float *
     ...
     **bfgsgrad1 = NULL,
     *bfgsscale1 =
         NULL; /* *bfgsscale2,*bfgsscale3;**bfgsmod2,**bfgsmod3,**bfgsgrad2,
                  **bfgsgrad3,*/
  ...
  float memdyn, memmodel, memseismograms, membuffer, memtotal,
      memcpml = 0.0, memdynf = 0.0, memgrad = 0.0, membfgs = 0.0;
    ...
    if (argc > 1) {
      ...
      if (strlen(FILEINP) > STRING_SIZE - 1) {
        fprintf(stderr,
                ...
                STRING_SIZE - 1);
        fprintf(stderr,
                " Error: IFOS could not read input parameter file name. -> "
                "Exit. \n\n");
        return -1;
      }
    } else {
      ...
      fprintf(stderr,
              " Caution: input parameter filename set to default 'ifos3d.inp'. "
              "\n\n");
    }
    FP = fopen(FILEINP, "r");
  ...
  if (MYID > 0) {
    ...
    LOG_INFO(FP," This is the log-file generated by PE %d \n\n",MYID);*/
    FP = fopen("/dev/null", "w");
    FI = fopen("/dev/null", "w");
  }
  ...
  if (MYID == 0) {
    FI = fopen("log/ifos3D_invers.out", "w");
  ...
  NT = (int)ceil(TIME /
                 DT); /* number of timesteps - replaces: NT=iround(TIME/DT); */
  TIME = (NT - 1) * DT; /* TIME set to true time of the last time step */
  if (NDTSHIFT > NT)
    ns = 0;
  else
    ns = (int)ceil(
        (float)(NT - NDTSHIFT) / (float)NDT); /* number of samples per trace -
                                                 replaces buggy formula:
                                                 ns=iround(NT-NDTSHIFT/NDT); */
  ...
  buffsize = (FDORDER)*4.0 * 6.0 * (max((NX * NZ), max((NY * NZ), (NX * NY)))) *
             sizeof(MPI_FLOAT);
  if (ABS_TYPE == 1)
    memcpml = 2.0 * FW * 6.0 * (NY * NZ + NX * NZ + NY * NX) * fac2 +
              24.0 * 2.0 * FW * fac2;
  if (METHOD) {
  ...
  memtotal = memdyn + memmodel + memseismograms + membuffer +
             (buffsize * pow(2.0, -20.0)) + memgrad + memdynf + membfgs +
             memcpml;
  ...
  if (MYID == 0) {
    ...
    LOG_INFO(FP, " Size of local grids: NX=%d \t NY=%d \t NZ=%d \n", NX, NY,
             NZ);
    LOG_INFO(FP, " Each process is now trying to allocate memory for:\n");
    LOG_INFO(FP, " Dynamic modeling variables: \t\t %6.2f MB\n", memdyn);
    LOG_INFO(FP, " Static modeling variables: \t\t %6.2f MB\n", memmodel);
    LOG_INFO(FP, " Seismograms: \t\t\t\t %6.2f MB\n", memseismograms);
    LOG_INFO(FP, " Dynamic inversion variables: \t\t %6.2f MB\n", memdynf);
    LOG_INFO(FP, " Static inversion variables: \t\t %6.2f MB\n",
             memgrad + membfgs);
    LOG_INFO(FP, " Buffer arrays for grid exchange: \t %6.2f MB\n", membuffer);
    LOG_INFO(FP, " Network Buffer for MPI_Bsend: \t\t %6.2f MB\n",
             buffsize * pow(2.0, -20.0));
    LOG_INFO(FP, " ------------------------------------------------ \n");
    LOG_INFO(FP, " Total memory required: \t\t %6.2f MB.\n\n", memtotal);
  }
  ...
  if (HESS && !READ_HESS)
    ntr_hess = ntr_glob / REC_HESS;
  else
    ntr_hess = 0;
  /* memory allocation for dynamic (wavefield) arrays */
  if (POS[2] == 0) {
    vx = f3tensor(0 - FDORDER / 2, NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                  NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                  NZ + l * FDORDER / 2);
    vy = f3tensor(0 - FDORDER / 2, NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                  NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                  NZ + l * FDORDER / 2);
    vz = f3tensor(0 - FDORDER / 2, NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                  NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                  NZ + l * FDORDER / 2);
    /*fvx  =
       f4tensor(1,NT,0-FDORDER/2,NY+l*FDORDER/2,1-l*FDORDER/2,NX+l*FDORDER/2,1-l*FDORDER/2,NZ+l*FDORDER/2);
            fvy  =
       f4tensor(1,NT,0-FDORDER/2,NY+l*FDORDER/2,1-l*FDORDER/2,NX+l*FDORDER/2,1-l*FDORDER/2,NZ+l*FDORDER/2);
            fvz  =
       f4tensor(1,NT,0-FDORDER/2,NY+l*FDORDER/2,1-l*FDORDER/2,NX+l*FDORDER/2,1-l*FDORDER/2,NZ+l*FDORDER/2);*/
    if (METHOD) {
      Ffvx = f4tensor(1, NFMAX, 0 - FDORDER / 2, NY + l * FDORDER / 2,
                      1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                      1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      Ffvy = f4tensor(1, NFMAX, 0 - FDORDER / 2, NY + l * FDORDER / 2,
                      1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                      1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      Ffvz = f4tensor(1, NFMAX, 0 - FDORDER / 2, NY + l * FDORDER / 2,
                      1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                      1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      Fbvx = f4tensor(1, NFMAX * (ntr_hess + 1), 0 - FDORDER / 2,
                      NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                      NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                      NZ + l * FDORDER / 2);
      Fbvy = f4tensor(1, NFMAX * (ntr_hess + 1), 0 - FDORDER / 2,
                      NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                      NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                      NZ + l * FDORDER / 2);
      Fbvz = f4tensor(1, NFMAX * (ntr_hess + 1), 0 - FDORDER / 2,
                      NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                      NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                      NZ + l * FDORDER / 2);
      Ffivx = f4tensor(1, NFMAX, 0 - FDORDER / 2, NY + l * FDORDER / 2,
                       1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                       1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      Ffivy = f4tensor(1, NFMAX, 0 - FDORDER / 2, NY + l * FDORDER / 2,
                       1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                       1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      Ffivz = f4tensor(1, NFMAX, 0 - FDORDER / 2, NY + l * FDORDER / 2,
                       1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                       1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      Fbivx = f4tensor(1, NFMAX * (ntr_hess + 1), 0 - FDORDER / 2,
                       NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                       NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                       NZ + l * FDORDER / 2);
      Fbivy = f4tensor(1, NFMAX * (ntr_hess + 1), 0 - FDORDER / 2,
                       NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                       NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                       NZ + l * FDORDER / 2);
      Fbivz = f4tensor(1, NFMAX * (ntr_hess + 1), 0 - FDORDER / 2,
                       NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                       NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                       NZ + l * FDORDER / 2);
    }

    sxy = f3tensor(0 - FDORDER / 2, NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                   NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                   NZ + l * FDORDER / 2);
    syz = f3tensor(0 - FDORDER / 2, NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                   NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                   NZ + l * FDORDER / 2);
  }

  if (POS[2] > 0) {
    vx = f3tensor(1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
    vy = f3tensor(1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
    vz = f3tensor(1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
    /*fvx  =
       f4tensor(1,NT,1-l*FDORDER/2,NY+l*FDORDER/2,1-l*FDORDER/2,NX+l*FDORDER/2,1-l*FDORDER/2,NZ+l*FDORDER/2);
            fvy  =
       f4tensor(1,NT,1-l*FDORDER/2,NY+l*FDORDER/2,1-l*FDORDER/2,NX+l*FDORDER/2,1-l*FDORDER/2,NZ+l*FDORDER/2);
            fvz  =
       f4tensor(1,NT,1-l*FDORDER/2,NY+l*FDORDER/2,1-l*FDORDER/2,NX+l*FDORDER/2,1-l*FDORDER/2,NZ+l*FDORDER/2);*/
    if (METHOD) {
      Ffvx = f4tensor(1, NFMAX, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                      1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                      1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      Ffvy = f4tensor(1, NFMAX, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                      1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                      1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      Ffvz = f4tensor(1, NFMAX, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                      1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                      1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      Fbvx = f4tensor(1, NFMAX * (ntr_hess + 1), 1 - l * FDORDER / 2,
                      NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                      NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                      NZ + l * FDORDER / 2);
      Fbvy = f4tensor(1, NFMAX * (ntr_hess + 1), 1 - l * FDORDER / 2,
                      NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                      NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                      NZ + l * FDORDER / 2);
      Fbvz = f4tensor(1, NFMAX * (ntr_hess + 1), 1 - l * FDORDER / 2,
                      NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                      NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                      NZ + l * FDORDER / 2);
      Ffivx = f4tensor(1, NFMAX, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                       1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                       1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      Ffivy = f4tensor(1, NFMAX, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                       1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                       1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      Ffivz = f4tensor(1, NFMAX, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                       1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                       1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      Fbivx = f4tensor(1, NFMAX * (ntr_hess + 1), 1 - l * FDORDER / 2,
                       NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                       NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                       NZ + l * FDORDER / 2);
      Fbivy = f4tensor(1, NFMAX * (ntr_hess + 1), 1 - l * FDORDER / 2,
                       NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                       NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                       NZ + l * FDORDER / 2);
      Fbivz = f4tensor(1, NFMAX * (ntr_hess + 1), 1 - l * FDORDER / 2,
                       NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                       NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                       NZ + l * FDORDER / 2);
    }
    sxy = f3tensor(1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                   1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                   1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
    syz = f3tensor(1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                   1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                   1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
  }

  sxz =
      f3tensor(1 - l * FDORDER / 2, NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
               NX + l * FDORDER / 2, 1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
  sxx =
      f3tensor(1 - l * FDORDER / 2, NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
               NX + l * FDORDER / 2, 1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
  syy =
  ...
  if (MYID == 0)
    LOG_INFO(FP, " ... memory allocation for PE %d was successfull.\n\n", MYID);
  ...
  if (MYID == 0) switch (SRCREC) {
      case 0:
        if (MYID == 0)
          err("SRCREC parameter is invalid (SRCREC!=1)! No source parameters "
              "specified!");
        break;
      case 1:
        LOG_INFO(
        ...
        while (fgets(buffer, STRING_SIZE, fpsrc)) {
          ...
          /* checks if the line contains a '%'character which indicates a
             comment line,
                                  and if the reading of a string was successful,
             which is not the case for an empty line*/
          if ((strchr(buffer, '%') == 0) &&
              (sscanf(buffer, "%s", bufferstring) == 1))
            ++(nsrc);
        }
        rewind(fpsrc);
        if ((nsrc) == 0)
          LOG_INFO(FP,
                   "\n WARNING: Could not determine number of sources "
                   "parameter sets in input file. Assuming %d.\n",
                   (nsrc = 0));
        else
          LOG_INFO(FP, " Number of source positions specified in %s : %d \n",
                   SOURCE_FILE, nsrc);
        break;
      case 2:
        if ((PLANE_WAVE_DEPTH > 0)) {
          /*determining the number of sources in the specified plane
           * normal/tilted to the surface/upper model boundary*/
          ...
          err("SRCREC parameter specifies PLANE_WAVE excitation, but "
              "PLANE_WAVE_DEPTH<=0!");
        }
        break;

      default:
        err("SRCREC parameter is invalid (SRCREC!=1 or SRCREC!=2)! No source "
  ...
  /*originally, SOURCE_TYPE (stype) is defined in the source file, if not,
   * SOURCE_TYPE is taken from the input file */
  ...
  if (ABS_TYPE == 1) {
    CPML_coeff(K_x, alpha_prime_x, a_x, b_x, K_x_half, alpha_prime_x_half,
               a_x_half, b_x_half, K_y, alpha_prime_y, a_y, b_y, K_y_half,
               alpha_prime_y_half, a_y_half, b_y_half, K_z, alpha_prime_z, a_z,
               b_z, K_z_half, alpha_prime_z_half, a_z_half, b_z_half);
  }
  ...
  if (HESS && READ_HESS == 1)
    readhess(NX, NY, NZ, hess1, hess2, hess3, finv[0],
             iteration + 1 - it_group);
  MPI_Barrier(MPI_COMM_WORLD);
  if (METHOD == 1 && EXTOBS == 1) {
    for (ishot = 1; ishot <= nshots; ishot++) {
      ...
      saveseis(FP, sectionvx, sectionvy, sectionvz, sectionp, sectioncurl,
               sectiondiv, recpos, recpos_loc, ntr, srcpos, ishot, ns, 1, 1);
    }
  }
  for (iteration = ITMIN; iteration <= ITMAX; iteration++) {
    ...
    if (METHOD) {
      ...
      zero_invers(NX, NY, NZ, Ffvx, Ffvy, Ffvz, Ffivx, Ffivy, Ffivz, Fbvx, Fbvy,
                  Fbvz, Fbivx, Fbivy, Fbivz, NFMAX, ntr_hess);
    }

    matcopy(rho, pi, u, taus, taup);
    if (FREE_SURF)
      constant_boundary(xb[0], xb[1], yb[0], yb[1], zb[0], zb[1], pi, u, rho);
    av_mat(rho, pi, u, taus, taup, uipjp, ujpkp, uipkp, tausipjp, tausjpkp,
           tausipkp, rjp, rkp, rip);
    L2all = 0.0;
    L2f = 0.0;
    misfit[0] = 0.0;
    ...
    if (MYID == 0) {
      fprintf(FI, "cdf=%d, f=%4.2f, itmax=%d \n", cdf, finv[0], itmax);
    }

    if (METHOD) {
      for (i = 1; i <= 4; i++) {
        for (j = 1; j <= ntr_loc; j++) {
          srcpos_loc_back[i][j] = recpos_loc[i][j];
        }
      }
    }
    ...
    for (ishot = 1; ishot <= nshots; ishot++) {
      ...
      if (RUN_MULTIPLE_SHOTS) {
        nsrc_loc = snum_loc[ishot];
        if (nsrc_loc > 0) {
          for (nt = 4; nt <= 7; nt++) srcpos_loc[nt][1] = srcpos[nt][ishot];
          srcpos_loc[1][1] =
              (float)(((iround(srcpos[1][ishot] / DX) - 1) % IENDX) + 1);
          srcpos_loc[2][1] =
              (float)(((iround(srcpos[2][ishot] / DY) - 1) % IENDY) + 1);
          srcpos_loc[3][1] =
              (float)(((iround(srcpos[3][ishot] / DZ) - 1) % IENDZ) + 1);
        }
      }
      ...
      zero_wavefield(NX, NY, NZ, vx, vy, vz, sxx, syy, szz, sxy, syz, sxz, rxx,
                     ryy, rzz, rxy, ryz, rxz, psi_sxx_x, psi_sxy_x, psi_sxz_x,
                     psi_sxy_y, psi_syy_y, psi_syz_y, psi_sxz_z, psi_syz_z,
                     psi_szz_z, psi_vxx, psi_vyx, psi_vzx, psi_vxy, psi_vyy,
                     psi_vzy, psi_vxz, psi_vyz, psi_vzz);
      if (METHOD)
        zero_invers(NX, NY, NZ, Ffvx, Ffvy, Ffvz, Ffivx, Ffivy, Ffivz, Fbvx,
                    Fbvy, Fbvz, Fbivx, Fbivy, Fbivz, NFMAX, 0);
      ...
      for (nt = 1; nt <= NT; nt++) {
        ...
        if (isnan(vy[NY / 2][NX / 2][NZ / 2]))
          err(" Simulation is unstable !"); /* maybe just breaking the loop
                                               would be better */

        /* update of particle velocities */
        time_v_update[nt] += update_v(
            xb[0], xb[1], yb[0], yb[1], zb[0], zb[1], nt, vx, vy, vz, sxx, syy,
            szz, sxy, syz, sxz, rho, rjp, rkp, rip, srcpos_loc, signals,
            signals, signals, nsrc_loc, absorb_coeff, 0);

        if (ABS_TYPE == 1) {
          update_v_CPML(xb[0], xb[1], yb[0], yb[1], zb[0], zb[1], nt, vx, vy,
                        vz, sxx, syy, szz, sxy, syz, sxz, rho, rjp, rkp, rip,
                        srcpos_loc, signals, signals, signals, nsrc_loc,
                        absorb_coeff, 0, K_x, a_x, b_x, K_x_half, a_x_half,
                        b_x_half, K_y, a_y, b_y, K_y_half, a_y_half, b_y_half,
                        K_z, a_z, b_z, K_z_half, a_z_half, b_z_half, psi_sxx_x,
                        psi_sxy_x, psi_sxz_x, psi_sxy_y, psi_syy_y, psi_syz_y,
                        psi_sxz_z, psi_syz_z, psi_szz_z);
        }

        /* exchange values of particle velocities at grid boundaries between PEs
         */
        time_v_exchange[nt] +=
            exchange_v(vx, vy, vz, bufferlef_to_rig, bufferrig_to_lef,
                       buffertop_to_bot, bufferbot_to_top, bufferfro_to_bac,
                       bufferbac_to_fro, req_send, req_rec);
        ...
        time_s_update[nt] += update_s_elastic(
            xb[0], xb[1], yb[0], yb[1], zb[0], zb[1], vx, vy, vz, sxx, syy, szz,
            sxy, syz, sxz, pi, u, uipjp, ujpkp, uipkp);
        ...
        if (ABS_TYPE == 1) {
          ...
          update_s_CPML_elastic(
              xb[0], xb[1], yb[0], yb[1], zb[0], zb[1], vx, vy, vz, sxx, syy,
              szz, sxy, syz, sxz, pi, u, uipjp, ujpkp, uipkp, K_x, a_x, b_x,
              K_x_half, a_x_half, b_x_half, K_y, a_y, b_y, K_y_half, a_y_half,
              b_y_half, K_z, a_z, b_z, K_z_half, a_z_half, b_z_half, psi_vxx,
              psi_vyx, psi_vzx, psi_vxy, psi_vyy, psi_vzy, psi_vxz, psi_vyz,
              psi_vzz);
        }
        ...
        time_s_exchange[nt] += exchange_s(
            sxx, syy, szz, sxy, syz, sxz, sbufferlef_to_rig, sbufferrig_to_lef,
            sbuffertop_to_bot, sbufferbot_to_top, sbufferfro_to_bac,
            sbufferbac_to_fro, sreq_send, sreq_rec);

        /* extracting monochromatic wavefields */
        if (nt % ntast == 0 && METHOD) {
          discfourier(1, NX, 1, NY, 1, NZ, nt, vx, vy, vz, Ffvx, Ffvy, Ffvz,
                      Ffivx, Ffivy, Ffivz, finv, nf, ntast, pshot, 0);
          l++;
        }
        ...
        if ((SEISMO) && (ntr > 0) && (nt == lsamp)) {
          seismo(nlsamp, ntr, recpos_loc, sectionvx, sectionvy, sectionvz,
                 sectiondiv, sectioncurl, sectionp, vx, vy, vz, sxx, syy, szz,
                 pi, u);
          nlsamp++;
          lsamp += NDT;
        }
        ...
        if ((SNAP) && (nt == lsnap) && (nt <= TSNAP2 / DT)) {
          MPI_Barrier(MPI_COMM_WORLD);
          snap(FP, nt, ++nsnap, SNAP_FORMAT, SNAP, vx, vy, vz, sxx, syy, szz, u,
               pi, IDX, IDY, IDZ, 1, 1, 1, NX, NY, NZ);
      ...
      if (SEISMO) {
        if (SAVESU) {
          saveseis(FP, sectionvx, sectionvy, sectionvz, sectionp, sectioncurl,
                   sectiondiv, recpos, recpos_loc, ntr, srcpos, ishot, ns, 0,
                   iteration);
        }
      }
      ...
      /*if (LOG)
                  if (MYID==0) timing(time_v_update,time_s_update,
         time_s_exchange,time_v_exchange,time_timestep, ishot);*/

      if (METHOD == 1) {
        MPI_Barrier(MPI_COMM_WORLD);
        if (METHOD) {
          /* exchange frequency domain wavefields */
          exchange_Fv(Ffvx, Ffvy, Ffvz, nf, bufferlef_to_rig, bufferrig_to_lef,
                      buffertop_to_bot, bufferbot_to_top, bufferfro_to_bac,
                      bufferbac_to_fro, req_send, req_rec, 0);
          exchange_Fv(Ffivx, Ffivy, Ffivz, nf, bufferlef_to_rig,
                      bufferrig_to_lef, buffertop_to_bot, bufferbot_to_top,
        ...
        if (ntr_loc > 0) {
          ...
          residual(sectionread, sectionreadf, sectionvx, sectionvxdiff, ntr_loc,
                   ns, &L2, &L2f);
          ...
          /* calculate residuals and L2 Norm */
          residual(sectionread, sectionreadf, sectionvy, sectionvydiff, ntr_loc,
                   ns, &L2, &L2f);
          ...
          /* calculate residuals and L2 Norm */
          residual(sectionread, sectionreadf, sectionvz, sectionvzdiff, ntr_loc,
                   ns, &L2, &L2f);
        }
        ...
        for (pshot = 0; pshot <= hloop; pshot++) {
          ...
          if (pshot > 0) { /*only for Hessian wavefields*/
            ...
            if (ntr_loc > 0) {
              for (i = 1; i <= 4; i++) {
                srcpos_loc_back[i][1] = recpos_loc[i][pshot_loc];
          ...
          zero_wavefield(NX, NY, NZ, vx, vy, vz, sxx, syy, szz, sxy, syz, sxz,
                         rxx, ryy, rzz, rxy, ryz, rxz, psi_sxx_x, psi_sxy_x,
                         psi_sxz_x, psi_sxy_y, psi_syy_y, psi_syz_y, psi_sxz_z,
                         psi_syz_z, psi_szz_z, psi_vxx, psi_vyx, psi_vzx,
                         psi_vxy, psi_vyy, psi_vzy, psi_vxz, psi_vyz, psi_vzz);
          ...
          for (nt = 1; nt <= NT; nt++) {
            ...
            /* update of particle velocities */
            time_v_update[nt] +=
                update_v(xb[0], xb[1], yb[0], yb[1], zb[0], zb[1], nt, vx, vy,
                         vz, sxx, syy, szz, sxy, syz, sxz, rho, rjp, rkp, rip,
                         srcpos_loc_back, sectionvxdiff, sectionvydiff,
                         sectionvzdiff, ntr_loc, absorb_coeff, 1);

            if (ABS_TYPE == 1) {
              update_v_CPML(xb[0], xb[1], yb[0], yb[1], zb[0], zb[1], nt, vx,
                            vy, vz, sxx, syy, szz, sxy, syz, sxz, rho, rjp, rkp,
                            rip, srcpos_loc_back, sectionvxdiff, sectionvydiff,
                            sectionvzdiff, ntr_loc, absorb_coeff, 1, K_x, a_x,
                            b_x, K_x_half, a_x_half, b_x_half, K_y, a_y, b_y,
                            K_y_half, a_y_half, b_y_half, K_z, a_z, b_z,
                            K_z_half, a_z_half, b_z_half, psi_sxx_x, psi_sxy_x,
                            psi_sxz_x, psi_sxy_y, psi_syy_y, psi_syz_y,
                            psi_sxz_z, psi_syz_z, psi_szz_z);
            }

            /* exchange values of particle velocities at grid boundaries between
             * PEs */
            time_v_exchange[nt] +=
                exchange_v(vx, vy, vz, bufferlef_to_rig, bufferrig_to_lef,
                           buffertop_to_bot, bufferbot_to_top, bufferfro_to_bac,
            ...
            time_s_update[nt] += update_s_elastic(
                xb[0], xb[1], yb[0], yb[1], zb[0], zb[1], vx, vy, vz, sxx, syy,
                szz, sxy, syz, sxz, pi, u, uipjp, ujpkp, uipkp);

            if (ABS_TYPE == 1) {
              update_s_CPML_elastic(
                  xb[0], xb[1], yb[0], yb[1], zb[0], zb[1], vx, vy, vz, sxx,
                  syy, szz, sxy, syz, sxz, pi, u, uipjp, ujpkp, uipkp, K_x, a_x,
                  b_x, K_x_half, a_x_half, b_x_half, K_y, a_y, b_y, K_y_half,
                  a_y_half, b_y_half, K_z, a_z, b_z, K_z_half, a_z_half,
                  b_z_half, psi_vxx, psi_vyx, psi_vzx, psi_vxy, psi_vyy,
                  psi_vzy, psi_vxz, psi_vyz, psi_vzz);
            }

            time_s_exchange[nt] += exchange_s(
                sxx, syy, szz, sxy, syz, sxz, sbufferlef_to_rig,
                sbufferrig_to_lef, sbuffertop_to_bot, sbufferbot_to_top,
                sbufferfro_to_bac, sbufferbac_to_fro, sreq_send, sreq_rec);

            if (nt % ntast == 0 && (METHOD)) {
              discfourier(1, NX, 1, NY, 1, NZ, nt, vx, vy, vz, Fbvx, Fbvy, Fbvz,
                          Fbivx, Fbivy, Fbivz, finv, nf, ntast, pshot1, 1);
              l++;
            }
            ...
            if ((SEISMO) && (ntr > 0) && (nt == lsamp)) {
              seismo(nlsamp, ntr, recpos_loc, sectionvx, sectionvy, sectionvz,
                     sectiondiv, sectioncurl, sectionp, vx, vy, vz, sxx, syy,
                     szz, pi, u);
              nlsamp++;
              lsamp += NDT;
            }
          ...
          if (!pshot) {
            exchange_Fv(Fbvx, Fbvy, Fbvz, nf, bufferlef_to_rig,
                        bufferrig_to_lef, buffertop_to_bot, bufferbot_to_top,
            ...
            gradient_F(NX, NY, NZ, Ffvx, Ffvy, Ffvz, Ffivx, Ffivy, Ffivz, Fbvx,
                       Fbvy, Fbvz, Fbivx, Fbivy, Fbivz, grad1, grad2, grad3, nt,
                       rho, pi, u, finv, nf, iteration);
          }
        } /*hloop*/
        if (iteration == 1 && HESS && !READ_HESS) {
          exchange_Fv(Fbvx, Fbvy, Fbvz, nf, bufferlef_to_rig, bufferrig_to_lef,
                      buffertop_to_bot, bufferbot_to_top, bufferfro_to_bac,
                      bufferbac_to_fro, req_send, req_rec, ntr_hess);
          exchange_Fv(Fbivx, Fbivy, Fbivz, nf, bufferlef_to_rig,
                      bufferrig_to_lef, buffertop_to_bot, bufferbot_to_top,
                      bufferfro_to_bac, bufferbac_to_fro, req_send, req_rec,
                      ntr_hess);
          MPI_Barrier(MPI_COMM_WORLD);
          hess_F(NX, NY, NZ, Ffvx, Ffvy, Ffvz, Ffivx, Ffivy, Ffivz, Fbvx, Fbvy,
                 Fbvz, Fbivx, Fbivy, Fbivz, hess1, hess2, hess3, nt, rho, pi, u,
                 finv, nf, ntr_hess);
        }
        hloop = 0;
        ntr_loc = ntr;
    ...
    if (METHOD) {
      ...
      if (HESS)
        hess_apply(1, NX, 1, NY, 1, NZ, grad1, grad2, grad3, hess1, hess2,
                   hess3, finv[0], iteration);

      /*preconditioning of gradient*/
      precon_grad(NX, NY, NZ, grad1, grad2, grad3, nsrc, srcpos, ntr_glob,
                  recpos, finv[0], iteration, cdf);
      if (GRADMO)
        outgrad(NX, NY, NZ, grad1, grad2, grad3, finv[0], iteration + 1000,
                GRAD_FILE);
      if (LBFGS) {
        if (it_group > 1) {
          ...
          lbfgs_savegrad(grad1, grad2, grad3, bfgsgrad1);
      }
      if (!LBFGS)
        conjugate(NX, NY, NZ, grad1, grad2, grad3, gradprior1, gradprior2,
                  gradprior3, gradprior4, gradprior5, gradprior6, beta,
                  iteration, cdf);
      if (GRADMO)
        outgrad(NX, NY, NZ, grad1, grad2, grad3, finv[0], iteration + 2000,
                GRAD_FILE);
      ...
      if (LBFGS || !LBFGS) {
        for (steptest = 1; steptest <= 2; steptest++) {
          ...
          modelupdate(NX, NY, NZ, grad1, grad2, grad3, testrho, testpi, testu,
                      bfgsmod1, step[steptest], beta, it_group);
          matcopy(testrho, testpi, testu, taus, taup);
          if (FREE_SURF)
            constant_boundary(xb[0], xb[1], yb[0], yb[1], zb[0], zb[1], testpi,
                              testu, testrho);
          av_mat(testrho, testpi, testu, taus, taup, uipjp, ujpkp, uipkp,
                 tausipjp, tausjpkp, tausipkp, rjp, rkp, rip);
          ...
          for (ishot = 1; ishot <= NSHOTS_STEP; ishot++) {
            ...
            if (RUN_MULTIPLE_SHOTS) {
              ...
              if (nsrc_loc > 0) {
                for (nt = 4; nt <= 7; nt++)
                  srcpos_loc[nt][1] = srcpos[nt][ishot1];
                srcpos_loc[1][1] =
                    (float)(((iround(srcpos[1][ishot1] / DX) - 1) % IENDX) + 1);
                srcpos_loc[2][1] =
                    (float)(((iround(srcpos[2][ishot1] / DY) - 1) % IENDY) + 1);
                srcpos_loc[3][1] =
                    (float)(((iround(srcpos[3][ishot1] / DZ) - 1) % IENDZ) + 1);
              }
            }
            ...
            zero_wavefield(NX, NY, NZ, vx, vy, vz, sxx, syy, szz, sxy, syz, sxz,
                           rxx, ryy, rzz, rxy, ryz, rxz, psi_sxx_x, psi_sxy_x,
                           psi_sxz_x, psi_sxy_y, psi_syy_y, psi_syz_y,
                           psi_sxz_z, psi_syz_z, psi_szz_z, psi_vxx, psi_vyx,
                           psi_vzx, psi_vxy, psi_vyy, psi_vzy, psi_vxz, psi_vyz,
                           psi_vzz);
            ...
            for (nt = 1; nt <= NT; nt++) {
              ...
              time_v_update[nt] += update_v(
                  xb[0], xb[1], yb[0], yb[1], zb[0], zb[1], nt, vx, vy, vz, sxx,
                  syy, szz, sxy, syz, sxz, testrho, rjp, rkp, rip, srcpos_loc,
                  signals, signals, signals, nsrc_loc, absorb_coeff, 0);

              if (ABS_TYPE == 1) {
                update_v_CPML(xb[0], xb[1], yb[0], yb[1], zb[0], zb[1], nt, vx,
                              vy, vz, sxx, syy, szz, sxy, syz, sxz, testrho,
                              rjp, rkp, rip, srcpos_loc, signals, signals,
                              signals, nsrc_loc, absorb_coeff, 0, K_x, a_x, b_x,
                              K_x_half, a_x_half, b_x_half, K_y, a_y, b_y,
                              K_y_half, a_y_half, b_y_half, K_z, a_z, b_z,
                              K_z_half, a_z_half, b_z_half, psi_sxx_x,
                              psi_sxy_x, psi_sxz_x, psi_sxy_y, psi_syy_y,
                              psi_syz_y, psi_sxz_z, psi_syz_z, psi_szz_z);
              };
              time_v_exchange[nt] += exchange_v(
                  vx, vy, vz, bufferlef_to_rig, bufferrig_to_lef,
                  buffertop_to_bot, bufferbot_to_top, bufferfro_to_bac,
                  bufferbac_to_fro, req_send, req_rec);
              ...
              time_s_update[nt] += update_s_elastic(
                  xb[0], xb[1], yb[0], yb[1], zb[0], zb[1], vx, vy, vz, sxx,
                  syy, szz, sxy, syz, sxz, testpi, testu, uipjp, ujpkp, uipkp);
              ...
              if (ABS_TYPE == 1) {
                update_s_CPML_elastic(
                    ...
                    K_y_half, a_y_half, b_y_half, K_z, a_z, b_z, K_z_half,
                    a_z_half, b_z_half, psi_vxx, psi_vyx, psi_vzx, psi_vxy,
                    psi_vyy, psi_vzy, psi_vxz, psi_vyz, psi_vzz);
              }
            ...
            if (ntr_loc > 0) {
              ...
              residual(sectionread, sectionreadf, sectionvx, sectionvxdiff,
                       ntr_loc, ns, &L2, &L2f);
              readseis(ishot1, sectionread, sectionreadf, ntr_loc, ns, 2);
              if (FILT == 1) {
                filt_seis(sectionread, ntr_loc, NT, finv[nf - 1]);
              }
              residual(sectionread, sectionreadf, sectionvy, sectionvydiff,
                       ntr_loc, ns, &L2, &L2f);
              readseis(ishot1, sectionread, sectionreadf, ntr_loc, ns, 3);
              if (FILT == 1) {
                filt_seis(sectionread, ntr_loc, NT, finv[nf - 1]);
              }
              residual(sectionread, sectionreadf, sectionvz, sectionvzdiff,
                       ntr_loc, ns, &L2, &L2f);
            }
        ...
        if (MYID == 0) LOG_INFO(FP, "\n\n Steplength Parabel \n");
        MPI_Barrier(MPI_COMM_WORLD);
        steplength(misfit, step, iteration,
                   it_group); /*find optimal steplength*/
      }
      if (MYID == 0) LOG_INFO(FP, "\n stepength calculation finished\n");

      modelupdate(NX, NY, NZ, grad1, grad2, grad3, rho, pi, u, bfgsmod1,
                  step[3], beta, it_group);
  ...
  if (POS[2] == 0) {
    free_f3tensor(vx, 0 - FDORDER / 2, NY + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
    free_f3tensor(vy, 0 - FDORDER / 2, NY + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
    free_f3tensor(vz, 0 - FDORDER / 2, NY + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
    free_f3tensor(sxy, 0 - FDORDER / 2, NY + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
    free_f3tensor(syz, 0 - FDORDER / 2, NY + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
    if (METHOD) {
      free_f4tensor(Ffvx, 1, NFMAX, 0 - FDORDER / 2, NY + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      free_f4tensor(Ffvy, 1, NFMAX, 0 - FDORDER / 2, NY + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      free_f4tensor(Ffvz, 1, NFMAX, 0 - FDORDER / 2, NY + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      free_f4tensor(Fbvx, 1, NFMAX * (ntr_hess + 1), 0 - FDORDER / 2,
                    NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NZ + l * FDORDER / 2);
      free_f4tensor(Fbvy, 1, NFMAX * (ntr_hess + 1), 0 - FDORDER / 2,
                    NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NZ + l * FDORDER / 2);
      free_f4tensor(Fbvz, 1, NFMAX * (ntr_hess + 1), 0 - FDORDER / 2,
                    NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NZ + l * FDORDER / 2);
      free_f4tensor(Ffivx, 1, NFMAX, 0 - FDORDER / 2, NY + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      free_f4tensor(Ffivy, 1, NFMAX, 0 - FDORDER / 2, NY + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      free_f4tensor(Ffivz, 1, NFMAX, 0 - FDORDER / 2, NY + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      free_f4tensor(Fbivx, 1, NFMAX * (ntr_hess + 1), 0 - FDORDER / 2,
                    NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NZ + l * FDORDER / 2);
      free_f4tensor(Fbivy, 1, NFMAX * (ntr_hess + 1), 0 - FDORDER / 2,
                    NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NZ + l * FDORDER / 2);
      free_f4tensor(Fbivz, 1, NFMAX * (ntr_hess + 1), 0 - FDORDER / 2,
                    NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NZ + l * FDORDER / 2);
    }
  ...
  if (POS[2] > 0) {
    free_f3tensor(vx, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
    free_f3tensor(vy, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
    free_f3tensor(vz, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
    free_f3tensor(sxy, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
    free_f3tensor(syz, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                  1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
    ...
    if (METHOD) {
      free_f4tensor(Ffvx, 1, NFMAX, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      free_f4tensor(Ffvy, 1, NFMAX, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      free_f4tensor(Ffvz, 1, NFMAX, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      free_f4tensor(Fbvx, 1, NFMAX * (ntr_hess + 1), 1 - l * FDORDER / 2,
                    NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NZ + l * FDORDER / 2);
      free_f4tensor(Fbvy, 1, NFMAX * (ntr_hess + 1), 1 - l * FDORDER / 2,
                    NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NZ + l * FDORDER / 2);
      free_f4tensor(Fbvz, 1, NFMAX * (ntr_hess + 1), 1 - l * FDORDER / 2,
                    NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NZ + l * FDORDER / 2);
      free_f4tensor(Ffivx, 1, NFMAX, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      free_f4tensor(Ffivy, 1, NFMAX, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      free_f4tensor(Ffivz, 1, NFMAX, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NX + l * FDORDER / 2,
                    1 - l * FDORDER / 2, NZ + l * FDORDER / 2);
      free_f4tensor(Fbivx, 1, NFMAX * (ntr_hess + 1), 1 - l * FDORDER / 2,
                    NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NZ + l * FDORDER / 2);
      free_f4tensor(Fbivy, 1, NFMAX * (ntr_hess + 1), 1 - l * FDORDER / 2,
                    NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NZ + l * FDORDER / 2);
      free_f4tensor(Fbivz, 1, NFMAX * (ntr_hess + 1), 1 - l * FDORDER / 2,
                    NY + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                    NZ + l * FDORDER / 2);
    }
  ...
  free_f3tensor(sxz, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                1 - l * FDORDER / 2, NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                NZ + l * FDORDER / 2);
  free_f3tensor(sxx, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                1 - l * FDORDER / 2, NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                NZ + l * FDORDER / 2);
  free_f3tensor(syy, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                1 - l * FDORDER / 2, NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                NZ + l * FDORDER / 2);
  free_f3tensor(szz, 1 - l * FDORDER / 2, NY + l * FDORDER / 2,
                1 - l * FDORDER / 2, NX + l * FDORDER / 2, 1 - l * FDORDER / 2,
                NZ + l * FDORDER / 2);
  ...
  if (MYID == 0) {
    ...
    time4 = MPI_Wtime();
    LOG_INFO(FP, " Total real time of program: %4.2f seconds.\n\n",
             time4 - time1);
    LOG_INFO(FP,
             " ***********************************************************\n");
    LOG_INFO(FP, " IFOS3D has finished.\n");
    LOG_INFO(
        FP, " ***********************************************************\n\n");
  }
