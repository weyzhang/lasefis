      ...
      for (j = ny1; j <= ny2; j++) {
        for (i = nx1; i <= nx2; i++) {
          for (k = nz1; k <= nz2; k++) {
            ...
            sxz_z =
                dz * (sxz[j][i][k] - sxz[j][i][k - 1]); /* R�ckw�rtsoperator */
        ...
        for (i = nx1; i <= nx2; i++) {
          for (k = nz1; k <= nz2; k++) {
            ...
            /*test_float = (unsigned int*) (void*) &vy[j][i][k];
                if(((*test_float & 0x7f800000) == 0x0) && ((*test_float &
               0x007fffff) != 0x0))fprintf(FP,"Achtung:ILLEGALER FLOAT
               %4.2f",vy[1000000][100000][1000000]);*/
...
printf("\n vx=%4.2f \n",vx[j][i][k]*1e100);}*/
    ...
    for (l = 1; l <= nsrc; l++) {
      ...
      switch ((int)srcpos_loc[7][l]) {
        case 2:
          ...
          /*/(rip[j][i][k]);  */ /* single force in x , DX^3 because of force
                                    density*/
        ...
        case 4:
          vy[j][i][k] +=
              amp * rkp[j][i][k]; /* single force in y, vertical direction*/
          break;
        case 5:
  ...
  /*if (LOG)
      if (MYID==0){
              time2=MPI_Wtime();
              time=time2-time1;
              fprintf(FP," Real time for particle velocity update: \t %4.2f
     s.\n",time);
      }*/
  return time;
}
