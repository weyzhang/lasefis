...
void sources(FILE* fpsrc, int* nsrc, float** srcpos) {
  ...
  if (MYID == 0) {
    ...
    for (l = 1; l <= *nsrc; l++) {
      ...
      switch (sscanf(cline, "%f%f%f%f%f%f%f", &xsrc, &ysrc, &zsrc, &tshift,
                     &srcpos[5][l], &srcpos[6][l], &srcpos[7][l])) {
        case 0:
          xsrc = 0.0;
        case 1:
          zsrc = 0.0;
...
void pwsources(int* nsrc, float** srcpos) { /* plane wave excitation */
  ...
  if (MYID == 0) {
    ...
    /*if (PHI==0.0) ifw=1;
       ...
       ixend=iround((((float)(FW*DX))+(((float)NYG*DY-((float)(FW*DX))-PLANE_WAVE_DEPTH)/tan_phi))/DX);
  ...
  /*if (MYID==0){
              fprintf(FP,"\n **Message from function source (written by PE
     %d):\n",MYID);
              fprintf(FP," Number of global source positions found:
     %i\n",*nsrc);
