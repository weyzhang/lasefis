...
void initproc(void) {
  ...
  if ((NPROC != NP) && (MYID == 0)) {
    fprintf(FP,
            "You specified NPROC =  %d (in parameter file) and NP = %d "
            "(command line) \n",
            NPROC, NP);
  ...
  if ((NX % NPROCX) > 0) err(" NX%NPROX must be zero  !");
  if ((NY % NPROCY) > 0) err(" NY%NPROY must be zero  !");
  if ((NZ % NPROCZ) > 0) err(" NZ%NPROZ must be zero  !");
  ...
  /*---------------   POS indicates the processor location in the 3D logical
   * processor array	---------*/
  POS[1] = MYID % NPROCX;              /*  x coordinate */
  POS[3] = (MYID / (NPROCX * NPROCY)); /*  z coordinate */
  POS[2] = ((MYID - (NPROCX * NPROCY) * POS[3]) /
            NPROCX); /* y coordinate (vertical) */
  ...
  /*if (POS[1] == 0)        INDEX[1]=INDEX[1] + NPROCX;
      ...
      if (POS[2] == NPROCY-1) INDEX[4]=MYID+NPROCX-(NPROCX*NPROCY);	only
     necessary for periodic boundary condition */
  ...
  fprintf(FP,
          " MYID \t POS(1):left,right \t POS(3): front, back \t POS(2): top, "
          "bottom\n");
  fprintf(FP, " %d \t\t %d: %d,%d \t\t %d: %d,%d \t\t %d: %d,%d \n", MYID,
          POS[1], INDEX[1], INDEX[2], POS[3], INDEX[5], INDEX[6], POS[2],
          INDEX[3], INDEX[4]);
