...
void checkfd(FILE* fp, float*** prho, float*** ppi, float*** pu, float*** ptaus,
  ...
  /*extern int RUN_MULTIPLE_SHOTS; no determination is done for the output check
     whether the simulation runs with one or multiple shot
  ...
  float cmax = 0.0, cmin = 1e9, sum, dtstab, ts, cmax_r, cmin_r, g = 0.0,
        gamma = 0.0;
  float srec_minx = DX * NX * NPROCX + 1, srec_miny = DY * NY * NPROCY + 1,
        srec_minz = DZ * NZ * NPROCZ + 1;
    ...
    for (i = 1 + nfw; i <= (nx - nfw); i++) {
      for (j = ny1; j <= (ny - nfw); j++) {
        ...
        for (l = 1; l <= L; l++) {
          ts = DT / peta[l];
          sum = sum +
                ((w * w * ptaus[j][i][k] * ts * ts) / (1.0 + w * w * ts * ts));
        }
        if (prho[j][i][k] == 0.0)
          printf("prho=0 detected from MYID=%d at i=%d,j=%d,k=%d\n", MYID, i, j,
                 k);
    ...
    for (i = 1 + nfw; i <= (nx - nfw); i++) {
      for (j = ny1; j <= (ny - nfw); j++) {
        ...
        for (l = 1; l <= L; l++) {
          ts = DT / peta[l];
          sum = sum +
                ((w * w * ptaup[j][i][k] * ts * ts) / (1.0 + w * w * ts * ts));
        }
        if (prho[j][i][k] == 0.0)
          printf("prho=0 detected from MYID=%d at i=%d,j=%d,k=%d\n", MYID, i, j,
                 k);
  ...
  fprintf(fp,
          ...
          "\n ");
  fprintf(fp,
          " MYID\t Vp_min(f=fc) \t Vp_max(f=inf) \t Vs_min(f=fc) \t "
          "Vsmax(f=inf) \n");
  fprintf(fp, " %d \t %8.2f \t %8.2f \t %8.2f \t %8.2f \n\n\n", MYID, cmin_p,
          cmax_p, cmin_s, cmax_s);
  ...
  /* estimate number of grid points per minimum wavelength to avoid numerical
   * dispersion */
  /* Taylor */
  if (FDCOEFF == 1) {
    switch (FDORDER) {
      ...
      case 12:
        ...
        break;
      default:
        err(" invalid FDORDER in checkfd_ssg() !");
        break;
    }
  }
  ...
  /* The original checks might delete files accidentally that would not be
     overwritten anyway.
         and did not test accessibility from all CPUs which may be vary,
     especially in distributed clusters */
  ...
  if (SNAP > 0) {
    ...
    switch (SNAP) {
      case 1:
        sprintf(xfile, "%s%s.x.%i%i%i", SNAP_FILE, file_ext, POS[1], POS[2],
                POS[3]);
        fprintf(fp, "    Check accessibility for snapshot file %s... \n",
                xfile);
        sprintf(errormessage, "PE %i cannot write snapshot %s!", MYID, xfile);
        if (((fpcheck = fopen(xfile, xmod)) == NULL))
          err(errormessage);
        else
          fclose(fpcheck);
        remove(xfile);

        sprintf(xfile, "%s%s.y.%i%i%i", SNAP_FILE, file_ext, POS[1], POS[2],
                POS[3]);
        fprintf(fp, "    Check accessibility for snapshot file %s... \n",
                xfile);
        sprintf(errormessage, "PE %i cannot write snapshot %s!", MYID, xfile);
        ...
        sprintf(xfile, "%s%s.z.%i%i%i", SNAP_FILE, file_ext, POS[1], POS[2],
                POS[3]);
        fprintf(fp, "    Check accessibility for snapshot file %s... \n",
                xfile);
        sprintf(errormessage, "PE %i cannot write snapshot %s!", MYID, xfile);
        ...
        break;
      case 2:
        sprintf(xfile, "%s%s.p.%i%i%i", SNAP_FILE, file_ext, POS[1], POS[2],
                POS[3]);
        // fprintf(fp,"    Check accessibility for snapshot file %s...
        // \n",xfile);
        sprintf(errormessage, "PE %i cannot write snapshot %s!", MYID, xfile);
      ...
      case 3:
        sprintf(xfile, "%s%s.div.%i%i%i", SNAP_FILE, file_ext, POS[1], POS[2],
                POS[3]);
        fprintf(fp, "    Check accessibility for snapshot file %s... \n",
                xfile);
        sprintf(errormessage, "PE %i cannot write snapshot %s!", MYID, xfile);
        ...
        sprintf(xfile, "%s%s.rot.%i%i%i", SNAP_FILE, file_ext, POS[1], POS[2],
                POS[3]);
        fprintf(fp, "    Check accessibility for snapshot file %s... \n",
                xfile);
        sprintf(errormessage, "PE %i cannot write snapshot %s!", MYID, xfile);
  ...
  if (SEISMO > 0) {
    ...
    /*if ((RUN_MULTIPLE_SHOTS)||(SEIS_FORMAT)) {
             possibly many files ... -> perform check of write and execute
       permission for directories, only
       ...
       (multiple shots) ... \n");
                    ...
                    case 1:  particle velocities only
                            ...
                            if (access(xfile,W_OK|X_OK)==-1) err2(" PE %d cannot
       write seismograms to %s!",xfile);
    ...
    switch (SEISMO) {
      case 1: /* particle velocities only */
        ...
        fprintf(fp, "    Check accessibility for seismogram file %s... \n",
                xfile); /*in case of number of PE's=500, there will be 500
                           messages, too many to be displayed! */
        sprintf(errormessage, "PE %i cannot write seismogram file %s!", MYID,
                xfile);
        // if (access(xfile,W_OK|X_OK)==-1) err(errormessage);
        if (((fpcheck = fopen(xfile, xmod)) == NULL) && (MYID == 0))
          err(errormessage);
        else
          fclose(fpcheck);
        remove(xfile);
        ...
        fprintf(fp, "    Check accessibility for seismogram file %s... \n",
                xfile);
        if (((fpcheck = fopen(xfile, xmod)) == NULL) && (MYID == 0))
          err("PE 0 cannot write seismograms!");
        else
          fclose(fpcheck);
        remove(xfile);
  ...
  fprintf(fp,
          " Here the minimum wavelength is assumed to be minimum model phase "
          "velocity \n");
  ...
  fprintf(fp,
          " The minimum wavelength (of S-waves) in the following simulation "
          "will\n");
  fprintf(fp, "    be %4.4f meter.\n", cmin / fmax);
  fprintf(fp,
          " Thus, at the chosen FD_ORDER of %i the recommended value for DH is "
  ...
  if ((dhmax > (cmin / fmax / g)) && (MYID == 0))
    warning(
        " Grid dispersion will influence wave propagation, choose smaller grid "
        "spacing (DH).");
  ...
  switch (FDORDER) {
    ...
    case 12:
      ...
      break;
    default:
      err(" invalid FDORDER in checkfd_acoustic() !");
      break;
  }
  ...
  fprintf(fp,
          " In this simulation the stability limit for timestep DT is %e "
          "seconds .\n",
          dtstab);
  ...
  if (SNAP) {
    ...
    if ((TSNAP2 > TIME) && (MYID == 0)) {
      sprintf(errormessage,
              "TSNAP2 = %e (last snapshot) > Time of wave propagation %e. "
              "Choose smaller TSNAP2.",
              TSNAP2, TIME);
      err(errormessage); /* if TSNAP2>simulation TIME, snapmerge will generate
                            "additional" snapshots out of nowhere, thus,
                            snapshot files size blow up */
    }
    ...
    fprintf(fp,
            " Output of snapshot gridpoints per node (NX/NPROCX/IDX) %8.2f .\n",
            snapoutx);
    fprintf(fp,
            " Output of snapshot gridpoints per node (NY/NPROCY/IDY) %8.2f .\n",
            snapouty);
    fprintf(fp,
            " Output of snapshot gridpoints per node (NZ/NPROCZ/IDZ) %8.2f .\n",
            snapoutz);
  ...
  if ((SEISMO > 0) && (MYID == 0)) {
    srec_minx = DX * NX * NPROCX + 1, srec_miny = DY * NY * NPROCY + 1,
    srec_minz = DZ * NZ * NPROCZ + 1;
    srec_maxx = -1.0, srec_maxy = -1.0, srec_maxz = -1.0;
    fprintf(fp,
            "\n Checking for receiver position(s) specified in input file.\n");
    fprintf(fp,
            "    Global grid size in m: %5.2f (x) : %5.2f (y) : %5.2f (z) :.\n",
            NX * DX * NPROCX, NY * DY * NPROCY, NZ * DZ * NPROCZ);
    if (ABS_TYPE) {
      if (FREE_SURF == 0)
        fprintf(fp,
                "    Global grid size in m(-width of abs.boundary) : \n        "
                "%5.2f-%5.2f (x in m) : %5.2f-%5.2f (y in m) : %5.2f-%5.2f (z "
                "in m).\n",
                FW * DX, NX * DX * NPROCX - FW * DX, FW * DY,
                NY * DY * NPROCY - FW * DY, FW * DZ,
                NZ * DZ * NPROCZ - FW * DZ);
      if (FREE_SURF == 1)
        fprintf(fp,
                "    Global grid size in m(-width of abs.boundary) : \n        "
                "%5.2f-%5.2f (x in m) : %5.2f-%5.2f (y in m) : %5.2f-%5.2f (z "
                "in m).\n",
                FW * DX, NX * DX * NPROCX - FW * DX, DY,
                NY * DY * NPROCY - FW * DY, FW * DZ,
                NZ * DZ * NPROCZ - FW * DZ);
    }

    /* find maximum and minimum source positions coordinate ---- from input
     * file*/
    ...
    if ((READREC == 1) || (READREC == 2)) {
      /* find maximum and minimum source positions coordinate ---- from receiver
       * file*/
      for (k = 1; k <= ntr; k++) {
      ...
      fprintf(fp,
              "    Number of receiver positions in receiver file %s : %i \n",
              REC_FILE, ntr);
    }

    fprintf(fp,
            "    Minimum receiver position coordinates : %5.2f (x) : %5.2f (y) "
            ": %5.2f (z).\n",
            srec_minx, srec_miny, srec_minz);
    fprintf(fp,
            "    Maximum receiver position coordinates : %5.2f (x) : %5.2f (y) "
            ": %5.2f (z).\n",
            srec_maxx, srec_maxy, srec_maxz);
    /* checking if receiver coordinate of first receiver in line specified in
     * input-file is inside the global grid */
    if ((((srec_maxx < 0.0) || (srec_maxy < 0.0)) || ((srec_maxz < 0.0))) ||
        (((srec_minx < 0.0) || (srec_miny < 0.0)) || ((srec_minz < 0.0)))) {
      err("\n\n Coordinate of at least one receiver location is outside the "
          "global grid. \n\n");
    }
    if (((srec_maxx > NX * DX * NPROCX) || (srec_maxy > NY * DY * NPROCY)) ||
        ((srec_maxz > NZ * DZ * NPROCZ))) {
      err("\n\n Coordinate of at least one receiver location is outside the "
          "global grid. \n\n");
    }
    /* checking if receiver coordinate of first receiver in line specified in
     * input-file is outside the Absorbing Boundary  */
    if (ABS_TYPE) {
      if ((((srec_maxx < (FW * DX)) || (srec_maxz < (FW * DZ))) ||
           ((srec_maxy < (FW * DY)) && !(FREE_SURF == 1))) ||
          (((srec_minx < (FW * DX)) || (srec_minz < (FW * DZ)))) ||
          ((srec_miny < (FW * DY)) && !(FREE_SURF == 1))) {
        /* this warning appears, when at least a single receiver is located in
         * AB between 0 - FW+DX/DX/DZ ("inner boundary")*/
        warning(
            "\n\n Coordinate of at least one receiver location is inside the "
            "Absorbing Boundary (warning 1). \n\n");
      }
      if (((srec_maxx > (NX * DX * NPROCX - FW * DX)) ||
           (srec_maxy > (NY * DY * NPROCY - FW * DY)) ||
           ((srec_maxz > (NZ * DZ * NPROCZ - FW * DZ))))) {
        /* this warning appears, when at least a single receiver is located in
         * AB between NX/NY/NZ-FW+DX/DX/DZ and NX/NY/NZ ("outer boundary")*/
        warning(
            "\n\n Coordinate of at least one receiver location is inside the "
            "Absorbing Boundary (warning 2). \n\n");
      }
    }

    fprintf(fp,
            " ... complete, receiver position specified in input file are "
            "located within the global grid.\n");
  }
  ...
  if ((SRCREC == 1) && (MYID == 0)) {
    ...
    fprintf(fp, "    Number of source positions in source file %s. : %i.\n",
            SOURCE_FILE, nsrc);
    fprintf(fp,
            "    Minimum source position coordinates : %5.2f (x) : %5.2f (y) : "
            "%5.2f (z).\n",
            srec_minx, srec_miny, srec_minz);
    fprintf(fp,
            "    Maximum source position coordinates : %5.2f (x) : %5.2f (y) : "
            "%5.2f (z).\n",
            srec_maxx, srec_maxy, srec_maxz);
    /* checking if receiver coordinate of first receiver in line specified in
     * input-file is inside the global grid */
    if ((((srec_maxx < 0.0) || (srec_maxy < 0.0)) || ((srec_maxz < 0.0))) ||
        (((srec_minx < 0.0) || (srec_miny < 0.0)) || ((srec_minz < 0.0)))) {
      err("\n\n Coordinate of at least one source location is outside the "
          "global grid. \n\n");
    }
    if (((srec_maxx > NX * DX * NPROCX) || (srec_maxy > NY * DY * NPROCY)) ||
        ((srec_maxz > NZ * DZ * NPROCZ))) {
    ...
    if (ABS_TYPE) {
      if ((((srec_maxx < (FW * DX)) || (srec_maxz < (FW * DZ))) ||
         ...
         * AB between 0 - FW+DX/DX/DZ ("inner boundary")*/
        warning(
            "\n\n Coordinate of at least one source location is inside the "
            "Absorbing Boundary (warning 1). \n\n");
      }
      if (((srec_maxx > (NX * DX * NPROCX - FW * DX)) ||
           (srec_maxy > (NY * DY * NPROCY - FW * DY)) ||
    ...
    fprintf(fp,
            " ...complete, all source position(s) specified in source file are "
            "located within the global grid.\n");
  }

  /* 		if ((NT/NDT-NDTSHIFT)>32000)
	err("\n\n Seismogram samples must be less than 32.0000 \n\n"); */ /*moved here from inside the previous
                                              statement for reason below */
  /* checked in writepar.c now. SU and SEG-Y allow 32767 samples, furthermore
      the exist programs allowing 65535 samples and pseudo-SEG-Y formats
     allowing
      almost arbitrarily long traces. And for binary and textual output the
     limit is
      arbitrary. */
  ...
  if (nsrc < NSHOTS_STEP) {
    ...
    if (MYID == 0) {
      warning(
          " Parameter NSHOTS_STEP is greater than number of sources. "
          "NSHOT_STEP is reduced to nsrc. \n");
