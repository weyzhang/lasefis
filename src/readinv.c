...
void readinv(float* finv, int* nf, int* groupnum, int* itpergroup, int nfmax) {
  ...
  *groupnum += 1;
  ...
  if (MYID == 0) {
    ...
    fscanf(fpinv, "%d%d%f%d%f%f%f%f%d", &itpergroup[0], &itpergroup[1], &abort,
           &mute, &twin[0], &twin[1], &owin[0], &owin[1], nf);

    fprintf(FP, "\n\n itmin=%d; itmax=%d; \n", itpergroup[0], itpergroup[1]);
    fprintf(FP, "\n Number of Frequencies in Stage Nr.%d: nf=%d \n\n",
            *groupnum, *nf);
    // fprintf(FP,"\n\n itmin=%d;itmax=%d;abort=%4.2f;mute=%d; twinmin=%4.2f;
    // twinmax=%4.2f;owinmin=%4.2f; owinmax=%4.2f; nf=%d
  ...
  if (nfmax > 1) {
    ...
    MPI_Bcast(&finv[0], 1, MPI_FLOAT, 0, MPI_COMM_WORLD);
  fprintf(FP, "\n");
