...
void matcopy(float*** rho, float*** pi, float*** u, float*** taus,
  ...
  MPI_Bsend(&buffertop_to_bot[0][0][1], (NX + 2) * (NZ + 2) * 5, MPI_FLOAT,
            INDEX[3], TAG5, MPI_COMM_WORLD);
  MPI_Barrier(MPI_COMM_WORLD);
  MPI_Recv(&buffertop_to_bot[0][0][1], (NX + 2) * (NZ + 2) * 5, MPI_FLOAT,
           INDEX[4], TAG5, MPI_COMM_WORLD, &status);
  MPI_Bsend(&bufferbot_to_top[0][0][1], (NX + 2) * (NZ + 2) * 5, MPI_FLOAT,
            INDEX[4], TAG6, MPI_COMM_WORLD);
  MPI_Barrier(MPI_COMM_WORLD);
  MPI_Recv(&bufferbot_to_top[0][0][1], (NX + 2) * (NZ + 2) * 5, MPI_FLOAT,
           INDEX[3], TAG6, MPI_COMM_WORLD, &status);
  ...
  MPI_Bsend(&bufferlef_to_rig[0][0][1], (NY + 2) * (NZ + 2) * 5, MPI_FLOAT,
            INDEX[1], TAG1, MPI_COMM_WORLD);
  MPI_Barrier(MPI_COMM_WORLD);
  MPI_Recv(&bufferlef_to_rig[0][0][1], (NY + 2) * (NZ + 2) * 5, MPI_FLOAT,
           INDEX[2], TAG1, MPI_COMM_WORLD, &status);
  MPI_Bsend(&bufferrig_to_lef[0][0][1], (NY + 2) * (NZ + 2) * 5, MPI_FLOAT,
            INDEX[2], TAG2, MPI_COMM_WORLD);
  MPI_Barrier(MPI_COMM_WORLD);
  MPI_Recv(&bufferrig_to_lef[0][0][1], (NY + 2) * (NZ + 2) * 5, MPI_FLOAT,
           INDEX[1], TAG2, MPI_COMM_WORLD, &status);
  ...
  MPI_Bsend(&bufferfro_to_bac[0][0][1], (NX + 2) * (NY + 2) * 5, MPI_FLOAT,
            INDEX[5], TAG3, MPI_COMM_WORLD);
  MPI_Barrier(MPI_COMM_WORLD);
  MPI_Recv(&bufferfro_to_bac[0][0][1], (NX + 2) * (NY + 2) * 5, MPI_FLOAT,
           INDEX[6], TAG3, MPI_COMM_WORLD, &status);
  MPI_Bsend(&bufferbac_to_fro[0][0][1], (NX + 2) * (NY + 2) * 5, MPI_FLOAT,
            INDEX[6], TAG4, MPI_COMM_WORLD);
  MPI_Barrier(MPI_COMM_WORLD);
  MPI_Recv(&bufferbac_to_fro[0][0][1], (NX + 2) * (NY + 2) * 5, MPI_FLOAT,
           INDEX[5], TAG4, MPI_COMM_WORLD, &status);
