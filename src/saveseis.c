...
void saveseis(FILE* fp, float** sectionvx, float** sectionvy, float** sectionvz,
              ...
              int ishot, int ns, int obs, int iteration) {
  ...
  char vxf[STRING_SIZE], vyf[STRING_SIZE], vzf[STRING_SIZE], curlf[STRING_SIZE],
      divf[STRING_SIZE], pf[STRING_SIZE], file_ext[5];
  ...
  if (ntr > 0) {
    if (RUN_MULTIPLE_SHOTS) {
      sprintf(vxf, "%s_vx_it%d.%s.shot%d.%d", seisfile, iteration, file_ext,
              ishot, MYID);
      sprintf(vyf, "%s_vy_it%d.%s.shot%d.%d", seisfile, iteration, file_ext,
              ishot, MYID);
      sprintf(vzf, "%s_vz_it%d.%s.shot%d.%d", seisfile, iteration, file_ext,
              ishot, MYID);
      sprintf(curlf, "%s_rot_it%d.%s.shot%d.%d", seisfile, iteration, file_ext,
              ishot, MYID);
      sprintf(divf, "%s_div_it%d.%s.shot%d.%d", seisfile, iteration, file_ext,
              ishot, MYID);
      sprintf(pf, "%s_p_it%d.%s.shot%d.%d", seisfile, iteration, file_ext,
              ishot, MYID);
    } else {
    ...
    switch (SEISMO) {
      case 1: /* particle velocities only */
        ...
        fprintf(fp,
                ...
                MYID, ntr, vxf);
        outseis(fp, fopen(vxf, "w"), 1, sectionvx, recpos, recpos_loc, ntr,
                srcpos1, nsrc, ns, SEIS_FORMAT);
        fprintf(fp, " PE %d is writing %d seismogramtraces (vy)   to \t %s \n",
                MYID, ntr, vyf);
        outseis(fp, fopen(vyf, "w"), 2, sectionvy, recpos, recpos_loc, ntr,
                srcpos1, nsrc, ns, SEIS_FORMAT);
        fprintf(fp, " PE %d is writing %d seismogramtraces (vz)   to \t %s \n",
                MYID, ntr, vzf);
        outseis(fp, fopen(vzf, "w"), 3, sectionvz, recpos, recpos_loc, ntr,
                srcpos1, nsrc, ns, SEIS_FORMAT);
      ...
      case 2: /* pressure only */
        fprintf(fp, " PE %d is writing %d seismogramtraces (p)   to \t %s \n",
                MYID, ntr, pf);
        outseis(fp, fopen(pf, "w"), 0, sectionp, recpos, recpos_loc, ntr,
                srcpos1, nsrc, ns, SEIS_FORMAT);
      ...
      case 3: /* curl and div only */
        ...
        fprintf(fp, " PE %d is writing %d seismogramtraces (div)   to \t %s \n",
                MYID, ntr, divf);
        outseis(fp, fopen(divf, "w"), 0, sectiondiv, recpos, recpos_loc, ntr,
                srcpos1, nsrc, ns, SEIS_FORMAT);
        fprintf(fp, " PE %d is writing %d seismogramtraces (curl)  to \t %s \n",
                MYID, ntr, curlf);
        outseis(fp, fopen(curlf, "w"), 0, sectioncurl, recpos, recpos_loc, ntr,
                srcpos1, nsrc, ns, SEIS_FORMAT);
  ...
  if (MYID == 0) {
    sprintf(outfile, "%s_vx_it%d.%s.shot%d", seisfile, iteration, file_ext,
            ishot);
    ...
    for (i = 0; i <= NPROC - 1; i++) {
      k = 0;
      sprintf(infile, "%s_vx_it%d.%s.shot%d.%d", seisfile, iteration, file_ext,
              ishot, i);
      fin = fopen(infile, "r");
      if (fin != 0) {
        ...
        if (k % (240 + 4 * ns) == 0)
          ...
          fprintf(FP, "attention: Error in seisfile");
          l = 0;
        }
        /*fprintf(FP,"l=%d",l);
  ...
  if (MYID == 0) {
    sprintf(outfile, "%s_vy_it%d.%s.shot%d", seisfile, iteration, file_ext,
            ishot);
    /*fprintf(FP,"outfile:%s",outfile);*/
    fout = fopen(outfile, "w");
    buf2 = vector(1, ns);
    ...
    for (i = 0; i <= NPROC - 1; i++) {
      sprintf(infile, "%s_vy_it%d.%s.shot%d.%d", seisfile, iteration, file_ext,
              ishot, i);
      fin = fopen(infile, "r");
      if (fin != 0) {
        fseek(fin, 0, SEEK_END);
  ...
  if (MYID == 0) {
    sprintf(outfile, "%s_vz_it%d.%s.shot%d", seisfile, iteration, file_ext,
            ishot);
    /*fprintf(FP,"outfile:%s",outfile);*/
    fout = fopen(outfile, "w");
    buf2 = vector(1, ns);
    ...
    k = 0;
    for (i = 0; i <= NPROC - 1; i++) {
      sprintf(infile, "%s_vz_it%d.%s.shot%d.%d", seisfile, iteration, file_ext,
              ishot, i);
      fin = fopen(infile, "r");
      if (fin != 0) {
        fseek(fin, 0, SEEK_END);
