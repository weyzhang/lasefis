...
/*------------------------------------------------------------------------
 *   smooth model parameters (complete model or boundaries only ), (not in
 * parallel!!)
 *  ----------------------------------------------------------------------*/
    ...
    for (k = dist + 1; k <= NZG - dist; k++) {
      ...
      for (i = dist + 1; i <= NXG - dist; i++) {
        ...
        for (j = dum + 1; j <= NYG - dist; j++) {
          ...
          dummy1 = mod1[j][dist][k] * exp(-a * (ii) * (ii)) +
                   mod1[j][i][k] * (1 - exp(-a * (ii) * (ii)));
          dummy2 = mod1[j][i][dist] * exp(-a * (kk) * (kk)) +
                   dummy1 * (1 - exp(-a * (kk) * (kk)));

          dummy1 = mod1[NYG - dist + 1][i][k] * exp(-a * (jjj) * (jjj)) +
                   dummy2 * (1 - exp(-a * (jjj) * (jjj)));
          dummy2 = mod1[j][NXG - dist + 1][k] * exp(-a * (iii) * (iii)) +
                   dummy1 * (1 - exp(-a * (iii) * (iii)));
          dummy1 = mod1[j][i][NZG - dist + 1] * exp(-a * (kkk) * (kkk)) +
                   dummy2 * (1 - exp(-a * (kkk) * (kkk)));
          mod1[j][i][k] = dummy1;
        }
      }
    }
