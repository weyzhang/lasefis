...
void outmod(int nx, int ny, int nz, float*** rho, float*** pi, float*** u,
  ...
  int i, j, k;
  char modfile1[STRING_SIZE], modfile2[STRING_SIZE], modfile3[STRING_SIZE],
      modfile4[STRING_SIZE];
  char modfile5[STRING_SIZE], modfile6[STRING_SIZE];
  float vp, vs;

  sprintf(modfile1, "%s.vp_it%d.%i.%i.%i", MOD_OUT_FILE, iteration, POS[1],
          POS[2], POS[3]);
  sprintf(modfile2, "%s.vs_it%d.%i.%i.%i", MOD_OUT_FILE, iteration, POS[1],
          POS[2], POS[3]);
  sprintf(modfile3, "%s.rho_it%d.%i.%i.%i", MOD_OUT_FILE, iteration, POS[1],
          POS[2], POS[3]);
