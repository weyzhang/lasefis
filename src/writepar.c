...
void writepar(FILE* fp, int ns) {
  ...
  extern int NX, NY, NZ, NT, SOURCE_SHAPE, SOURCE_TYPE, FDORDER,
      RUN_MULTIPLE_SHOTS;
  ...
  extern char SNAP_FILE[STRING_SIZE], SOURCE_FILE[], SIGNAL_FILE[], REC_FILE[],
      SEIS_FILE[STRING_SIZE];
  extern char MFILE[STRING_SIZE], GRAD_FILE[STRING_SIZE], INV_FILE[STRING_SIZE],
      SEIS_OBS_FILE[STRING_SIZE];
  ...
  extern int ITMIN, ITMAX, FILT, NFMAX, TAST, NSHOTS_STEP, DAMPTYPE, HESS,
      READ_HESS, REC_HESS, LBFGS, EXTOBS;
  ...
  if ((SRCREC) && (!PLANE_WAVE_DEPTH)) {
    ...
    if (RUN_MULTIPLE_SHOTS) {
      fprintf(fp,
              "\n IFOS will run (independent) simulations for each source "
              "defined in %s\n\n",
              SOURCE_FILE);
    }
  ...
  switch (SOURCE_TYPE) {
    case 1:
      fprintf(fp,
              " explosive point source (concentrated at a single gridpoint)\n");
      break;
    ...
    case 4:
      fprintf(
          fp,
          " point source with directive force in  y-direction (vertical) \n");
      break;
    ...
    default:
      fprintf(fp,
              " WARNING: Default type of source ('%d') not available -> "
              "changed to explosive! ",
              SOURCE_TYPE);
      SOURCE_TYPE = 1;
  }
    ...
    switch (READREC) {
      ...
      case 2:
        ...
        fprintf(fp, " Vertical increment between planes: %e m \n",
                REC_ARRAY_DIST);
        fprintf(fp,
                " Distance between receivers in x-direction within plane: %i "
                "Gridpoints\n",
                DRX);
        fprintf(fp,
                " Distance between receivers in z-direction within plane: %i "
                "Gridpoints\n",
                DRZ);
        fprintf(fp, "\n");
        break;
      ...
      case 0:
        ...
        fprintf(fp,
                " First receiver position (XREC1,YREC1,ZREC1) = (%5.3f, %5.3f, "
                "%5.3f m\n",
                XREC1, YREC1, ZREC1);
        fprintf(fp,
                " Last receiver position (XREC2,YREC2,ZREC2)  = (%5.3f, %5.3f, "
                "%5.3f) m\n",
                XREC2, YREC2, ZREC2);
        fprintf(fp, "\n");
        break;
  ...
  if (FW > 0) {
    ...
    fprintf(fp,
            " The percentage of amplitude decay at the edge is set to %f \n",
            DAMPING);

  } else {
    fprintf(fp, " Absorbing frame not installed ! \n");
    fprintf(fp,
            " Be aware of artificial reflections from the edges of the "
            "numerical mesh ! \n");
  }
  ...
  switch (BOUNDARY) {
    ...
    case 1:
      fprintf(fp,
              " Periodic boundary condition at left/right and front/back sides "
              "of global grid.\n");
      break;

    default:
      warning(
          " Wrong integer value for BOUNDARY specified in parameter file! ");
  ...
  if (SNAP) {
    ...
    switch (SNAP_PLANE) {
      ...
      case 2:
        fprintf(fp,
                " \nDiv and curl output will be as Energy with sign true for "
                "xy-plane. \n");
        break;
  ...
  if (SEISMO) {
    ...
    if ((SEISMO == 3) || (SEISMO == 4)) {
      fprintf(fp,
              " Seismograms of curl (S-wave component) and div (P-wave "
              "component of wavefield).\n");
      fprintf(fp, " output-files: \n ");
      fprintf(fp, "\t%s_rot.%s \n\t%s_div.%s\n", SEIS_FILE, file_ext, SEIS_FILE,
              file_ext);
    }
    ...
    if (ns) {
      ...
      fprintf(fp,
              " Amplitudes will be written every %d%s time-step, starting at "
              "the %d%s.\n",
    ...
    switch (SEIS_FORMAT) {
      ...
      case 1:
        fprintf(fp,
                " Seismograms are written in SU-format (output in meter, "
                "native endian and floats). \n");
        break;
  ...
  if (METHOD) {
    ...
    fprintf(fp,
            ...
            TAST);
    fprintf(fp,
            " \n average_model_parameter VP0=%5.2f m/s, VS0=%5.2f m/s, "
            "RHO0=%5.2f kg/m^3\n",
            VP0, VS0, RHO0);
    fprintf(fp,
            " \n parameter_class_weighting_factors_for_vp: %5.2f, vs: %5.2f, "
            "rho: %5.2f\n",
