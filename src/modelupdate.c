...
#include "fd.h"
void modelupdate(int nx, int ny, int nz, float*** gradvp, float*** gradvs,
                 float*** gradrho, float*** rho, float*** pi, float*** u,
                 float** bfgsmod1, float step, float* beta, int it_group) {
  extern FILE* FP;
  ...
  if (!LBFGS) {
    ...
    for (j = 1; j <= ny; j++) {
      for (i = 1; i <= nx; i++) {
        for (k = 1; k <= nz; k++) {
          ...
          if (fabs(gradrho[j][i][k]) > max1[2])
            max1[2] = fabs(gradrho[j][i][k]);
        }
      }
    }
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    if (max1[0] > 0.0 && max1[1] > 0.0) {
      ...
      fprintf(FP,
              "Warning: One of the gradients is zero: max1*max2*max3 =0 \n");
  }
  ...
  for (j = 1; j <= ny; j++) {
    for (i = 1; i <= nx; i++) {
      for (k = 1; k <= nz; k++) {
        ...
        vpnew = sqrt(pi[j][i][k] / rho[j][i][k]) +
                step * scale1 * gradvp[j][i][k] * VP0;

        vsnew = 0.0;
        /*vsnew=sqrt(u[j][i][k]/rho[j][i][k])+max[1]*step*gradconvs[j][i][k];*/
        vsnew = sqrt(u[j][i][k] / rho[j][i][k]) +
                step * scale2 * gradvs[j][i][k] * VS0;
