...
int read_objects_from_intputfile(FILE* fp, char* input_file,
                                 char** varname_list, char** value_list) {
  char errormessage[STRING_SIZE2];
  char varname_tmp1[STRING_SIZE2], varname_tmp2[STRING_SIZE2],
      varname_tmp3[STRING_SIZE2];
  char varname_tmp4[STRING_SIZE2], varname_tmp5[STRING_SIZE2];
  char value_tmp1[STRING_SIZE2], value_tmp2[STRING_SIZE2],
      value_tmp3[STRING_SIZE2];
    ...
    if (((strstr(cline, ":")) && ((strstr(cline, "\"")))) &&
      ...
      switch (occurence_doublequotes) {
        case 4:
          ...
          // up to 5 objects can be defined per line, more can be implemented
          // here
          switch (occurence_commas) {
            case 0:  // only a single object (name+value) in line
              ...
              if (sscanf(cline, " \"%[^\"]\" : \"%[^\"]\"", varname_tmp1,
                         value_tmp1) != 2) {
                sprintf(errormessage,
                        "Error in Input file, line %i, cannot read object name "
                        "and object value !",
                        lineno);
                err(errormessage);
              }
              ...
              add_object_tolist(varname_tmp1, value_tmp1, &number_readobject,
            ...
            case 3:  // two objects (name+value) in line
              ...
              // extract object name + object value from the line-string
              if (sscanf(cline, " \"%[^,],%[^\"]\" : \"%[^,],%[^\"]\"",
                         varname_tmp1, varname_tmp2, value_tmp1,
                         value_tmp2) != 4) {
                sprintf(errormessage,
                        "Error in Input file, line %i, cannot read two object "
                        "names and values !",
                        lineno);
                err(errormessage);
              }
              ...
              add_object_tolist(varname_tmp1, value_tmp1, &number_readobject,
                                varname_list, value_list);
              add_object_tolist(varname_tmp2, value_tmp2, &number_readobject,
                                varname_list, value_list);
            ...
            case 5:  // three objects (name+value) in line
              ...
              if (sscanf(cline,
                         " \"%[^,],%[^,],%[^\"]\" : \"%[^,],%[^,],%[^\"]\"",
                         varname_tmp1, varname_tmp2, varname_tmp3, value_tmp1,
                         value_tmp2, value_tmp3) != 6) {
                sprintf(errormessage,
              ...
              add_object_tolist(varname_tmp2, value_tmp2, &number_readobject,
                                varname_list, value_list);
              add_object_tolist(varname_tmp3, value_tmp3, &number_readobject,
                                varname_list, value_list);
            ...
            case 9:  // five objects (name+value) in line
              ...
              if (sscanf(cline,
                         ...
                         "\"%[^,],%[^,],%[^,],%[^,],%[^\"]\"",
                         varname_tmp1, varname_tmp2, varname_tmp3, varname_tmp4,
                         varname_tmp5, value_tmp1, value_tmp2, value_tmp3,
                         value_tmp4, value_tmp5) != 10) {
                sprintf(errormessage,
                        "Error in Input file, line %i, cannot read three "
                        "object names and values !",
                        lineno);
              ...
              add_object_tolist(varname_tmp4, value_tmp4, &number_readobject,
                                varname_list, value_list);
              add_object_tolist(varname_tmp5, value_tmp5, &number_readobject,
                                varname_list, value_list);

              // very strange: code crashes if both lines are commented here!
              // this only effects the last case of the switch!
              // should though not affect anything as long as number_readobject
              // keeps its value
              // in this case a new object is allocated which is already
              // there...

              // varname_list = malloc(sizeof(*varname_list));
              // value_list = malloc(sizeof(*value_list));
              varname_list[number_readobject] =
                  malloc(STRING_SIZE * sizeof(char*));

              // varname_list[number_readobject] =
              // (char**)malloc(STRING_SIZE*sizeof(char*));
              // value_list[number_readobject] =
              // (char**)malloc(STRING_SIZE*sizeof(char*));
            ...
            default:
              sprintf(errormessage,
                      "Error in Input file, line %i, only 0, 1, 3, 5, 7 or 9 "
                      "commas are allowed per line, but found %i !",
        ...
        default:
          sprintf(errormessage,
                  "Error in Input file, line %i, only 4 (two pairs) of double "
                  "quotes are allowed per line, but found %i !",
      ...
      // \n",lineno,number_readobject,
      // varname_list[number_readobject-1],value_list[number_readobject-1]);
    }
  }
...
int count_occure_charinstring(char stringline[STRING_SIZE2],
                              char teststring[]) {
...
void copy_str2str_uptochar(char string_in[STRING_SIZE2],
                           char string_out[STRING_SIZE2], char teststring[]) {
  int ii = 0;
...
int get_int_from_objectlist(char string_in[STRING_SIZE2], int number_readobject,
  ...
  while ((strcmp(varname_list[ii], string_in) != 0) &&
         ((ii + 1) < number_readobject)) {
    ii++;
  }
  ...
  if (strcmp(varname_list[ii], string_in) == 0) {
    ...
    if (strlen(value_list[ii]) == 0) {
      sprintf(errormessage, "Error in Input file, value of object %s is empty!",
              string_in);
      err(errormessage);
    }
    ...
    if (strlen(string_buffer) > 0) {
      ...
      sprintf(errormessage,
              "Error in Input file, value of object %s contains more than one "
              "float: '%s'!",
    ...
    if ((modf(double_buffer, &intpart)) == 0) {
      *int_buffer = atoi(value_list[ii]);
      ...
      sprintf(errormessage,
              "Error in Input file, value of object %s is not an int : %f !",
              string_in, double_buffer);
      err(errormessage);
      *int_buffer = -1;
...
int get_float_from_objectlist(char string_in[STRING_SIZE2],
  ...
  if (strcmp(varname_list[ii], string_in) == 0) {
    ...
    // \n",string_buffer,strlen(string_buffer));
    if ((strlen(string_buffer) == 0) ||
        ((strlen(string_buffer) > 0) &&
         ((is_string_blankspace(string_buffer)) == 1))) {
...
int get_string_from_objectlist(char string_in[STRING_SIZE2],
                               int number_readobject,
                               char string_buffer[STRING_SIZE2],
                               char** varname_list, char** value_list) {
  int ii = 0, checkifstringfound = 1;
  char errormessage[STRING_SIZE2];
...
void remove_blankspaces_around_string(char string_in[STRING_SIZE2]) {
  ...
  // copy string content from input string (ignoring blank spaces before and
  // afer string)
...
void add_object_tolist(char string_name[STRING_SIZE2],
                       char string_value[STRING_SIZE2], int* number_readobject,
                       char** varname_list, char** value_list) {
  // problem: the sscanf line reads between double quotes, including blank space
  // therefore another sscanf is performed in function
  // remove_blankspace_around_string
  ...
  //	varname_list[*number_readobject] =
  //(char**)malloc(STRING_SIZE2*sizeof(char*));
  //	value_list[*number_readobject] =
  //(char**)malloc(STRING_SIZE2*sizeof(char*));
